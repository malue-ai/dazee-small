...
class ToolHandler:
    ...
    统一接口：execute(params, context)
    """

    async def execute(self, params: Dict[str, Any], context: Optional[Any] = None) -> Any:
        """
        执行工具
    ...
    async def execute_streaming(
        self, params: Dict[str, Any], context: Optional[Any] = None
        ...
        yield ToolResultChunk(
            invocation_id=params.get("_invocation_id", ""),
            chunk_index=0,
            chunk_type="data",
            content=result,
        )
...
class ToolService:
    ...
    def register_from_request(self, request: ToolRegistration) -> ToolRegistrationResponse:
        """
        从注册请求注册工具
        ...
        return self.register_tool(
            definition=request.definition, replace_existing=request.replace_existing
        )
    ...
    def register_function(
        ...
        **kwargs,
        ...
        definition = ToolDefinition(
            ...
            input_schema=(
                ToolInputSchema(**input_schema) if isinstance(input_schema, dict) else input_schema
            ),
            **kwargs,
        )
    ...
    def list_tools(self, query: Optional[ToolListQuery] = None) -> ToolListResponse:
        """
        列出工具
        ...
        if query:
            ...
            return ToolListResponse(
                tools=tools, total=total, page=query.page, page_size=query.page_size
            )

        return ToolListResponse(tools=tools, total=len(tools), page=1, page_size=len(tools))
    ...
    def get_claude_tool_schemas(
        ...
        return [tool.to_claude_schema() for tool in tools if tool.status == ToolStatus.AVAILABLE]
        ...
        try:
            if definition.tool_type == ToolType.USER_DEFINED:
                ...
                raise ToolExecutionError(
                    ...
                    f"MCP 工具请通过 InstanceRegistry 执行。"
                )
    ...
    async def execute_tool_streaming(
        ...
        try:
            ...
            yield ToolResultChunk(
                invocation_id=invocation_id, chunk_index=0, chunk_type="error", content=str(e)
            )

    # ==================== 内部方法 ====================

    async def _execute_user_defined_tool(
        self, tool_name: str, input_data: Dict[str, Any], invocation_id: str
    ) -> Any:
        """执行 User-defined 工具"""
        handler = self._handlers.get(tool_name)
        if not handler:
    ...
    def _load_handler(self, implementation: Dict[str, str]) -> ToolHandler:
        ...
        try:
            ...
            # 包装为 ToolHandler（统一接口：execute(params, context)）
            class WrappedHandler(ToolHandler):
                ...
                async def execute(
                    ...
                    return await self._tool.execute(params, context or ToolContext())
...
def tool(
    name: str = None, description: str = "", return_mode: ReturnMode = ReturnMode.DIRECT, **kwargs
):
    """
    工具注册装饰器
    ...
    def decorator(func: Callable):
        ...
        service.register_function(
            name=tool_name, description=tool_desc, func=func, return_mode=return_mode, **kwargs
        )
