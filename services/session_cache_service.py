...
class SessionCacheService:
    ...
    def __init__(
        ...
        logger.info(f"✅ SessionCacheService 已初始化: max_context_size={max_context_size}")
    ...
    async def warmup_context(
        self, conversation_id: str, limit: int = 50, force: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if not force and conversation_id in self._active_sessions:
            logger.info("✅ 会话上下文已存在（无需预加载）: conversation_id=%s", conversation_id)
            return {
        ...
        return {"context": context, "cache_hit": False, "effective_limit": effective_limit}
    ...
    async def _load_from_db(self, conversation_id: str, limit: int = 50) -> ConversationContext:
        ...
        result = await self.conversation_service.get_conversation_messages(
            conversation_id=conversation_id, limit=limit, order="desc"  # 从新到旧
        )
    ...
    async def get_messages_for_llm(self, conversation_id: str) -> List[Dict[str, Any]]:
        """
        ...
        for msg_ctx in context.messages:
            ...
            llm_messages.append({"role": msg_ctx.role, "content": content_blocks})
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        total_messages = sum(len(ctx.messages) for ctx in self._active_sessions.values())
