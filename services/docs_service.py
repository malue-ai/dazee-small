...
from models.docs import DocFile, DocCategory, DocsStructure, DocContent
...
DOCS_CATEGORIES = {
    ...
    "api": {
        "name": "API 文档",
        "icon": "🔌",
        "description": "API 规范和接口说明"
    },
    "guides": {
        "name": "使用指南",
        "icon": "📖",
        "description": "功能使用和配置指南"
    },
    "deployment": {
        "name": "部署文档",
        "icon": "🚀",
    ...
    "specs": {
        "name": "技术规范",
        "icon": "📋",
        "description": "技术规范和标准定义"
    },
    "reports": {
        "name": "分析报告",
        "icon": "📊",
        "description": "分析报告和优化建议"
    },
    "analysis": {
        "name": "流程分析",
        "icon": "🔍",
        "description": "系统流程分析和性能优化"
    },
    "troubleshooting": {
        "name": "故障排查",
        "icon": "🔧",
    ...
    for category_id, config in DOCS_CATEGORIES.items():
        ...
        for md_file in sorted(category_path.glob("*.md")):
            ...
            files.append(DocFile(
                name=md_file.name,
                path=f"{category_id}/{md_file.name}",
                title=title
            ))
            total_files += 1
