...
class MCPClientWrapper:
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                logger.info(
                    f"🔌 连接 MCP 服务器: {self.server_url} (超时: {self.connect_timeout}s)"
                )
                ...
                try:
                    ...
                    self._httpx_client = httpx.AsyncClient(
                        headers=headers, timeout=httpx.Timeout(self.tool_timeout)
                    )

                    # 使用 Streamable HTTP 客户端连接（新版 API：传入 http_client）
                    self._http_cm = streamable_http_client(
                        self.server_url, http_client=self._httpx_client
                    )
                    read_stream, write_stream, _ = await asyncio.wait_for(
                        self._http_cm.__aenter__(), timeout=self.connect_timeout
                    )
                    ...
                    # 初始化会话（带超时）
                    await asyncio.wait_for(self._session.__aenter__(), timeout=self.connect_timeout)

                    # 发送 initialize 请求（带超时）
                    result = await asyncio.wait_for(
                        self._session.initialize(), timeout=self.connect_timeout
                    )
                ...
                logger.warning(
                    f"⚠️ MCP 连接失败 (尝试 {attempt + 1}/{self.max_retries + 1}): {error_type}: {str(e)}"
                )
                last_error = e
                # 清理可能残留的资源
                await self._cleanup_connection()
        ...
        # 所有重试都失败
        logger.error(
            f"❌ MCP 服务器连接彻底失败（已重试 {self.max_retries} 次）: {str(last_error)}"
        )
        self._connected = False
        self._connection_failed = True
        return False
    ...
    async def discover_tools(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                result = await asyncio.wait_for(
                    self._session.list_tools(), timeout=30.0  # 30 秒超时
                )
            except asyncio.TimeoutError:
            ...
            for tool in result.tools:
                ...
                logger.info(
                    f"   🔍 工具 {tool.name} 原始 inputSchema: {getattr(tool, 'inputSchema', 'N/A')}"
                )
                schema_props = (
                    input_schema.get("properties", {}) if isinstance(input_schema, dict) else {}
                )
                logger.info(
                    f"   📋 工具 {tool.name} 解析后 input_schema 参数: {list(schema_props.keys()) if schema_props else '(空)'}"
                )
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self._connected or not self._session:
            ...
            return {"success": False, "error": "MCP 连接已断开，请重试（系统会自动重连）"}
        ...
        try:
            ...
            expected_params = (
                list(expected_schema.get("properties", {}).keys()) if expected_schema else []
            )
            ...
            try:
                result = await asyncio.wait_for(
                    self._session.call_tool(original_name, arguments), timeout=self.tool_timeout
                )
            except asyncio.TimeoutError:
            ...
            if (
                ...
                or error_type == "ClosedResourceError"
            ):
                self._connected = False
                self._session = None
...
def create_mcp_tool_definition(
    tool_info: Dict[str, Any], client: "MCPClientWrapper"
) -> Dict[str, Any]:
    """
...
async def quick_mcp_call(
    ...
    Returns:
        执行结果
    """
    client = MCPClientWrapper(server_url=server_url, server_name=server_name, auth_token=auth_token)
...
async def test_mcp_connection():
    ...
    client = MCPClientWrapper(server_url=server_url, server_name="dify", auth_token=auth_token)
    ...
    try:
        ...
        if tools:
            ...
            result = await client.call_tool(
                first_tool["name"], {"query": "用户管理系统包含用户、角色、权限"}
            )
            print(f"📊 调用结果: {result}")
