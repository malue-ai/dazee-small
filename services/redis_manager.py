...
class RedisSessionManager:
    ...
    async def _get_client(self) -> redis.Redis:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                await asyncio.wait_for(self._client.ping(), timeout=3.0)  # 每次 ping 最多等 3 秒
                if attempt > 1:
                ...
                last_error = e
                logger.warning(f"⚠️ Redis 连接失败 (尝试 {attempt}/{max_retries}): {str(e)}")
                if attempt < max_retries:
    ...
    async def create_session(
        ...
        # 保存 Session 状态
        await client.hset(f"session:{session_id}:status", mapping=session_status)
    ...
    async def update_session_status(self, session_id: str, **fields) -> None:
        """
        更新 Session 状态
        ...
        await client.hset(f"session:{session_id}:status", mapping=str_fields)
    ...
    async def complete_session(self, session_id: str, status: str = "completed") -> None:
        ...
        Args:
            session_id: Session ID
            status: 最终状态（completed/failed/timeout/stopped）
        """
        client = await self._get_client()

        # 更新状态
        await self.update_session_status(
            session_id, status=status, last_heartbeat=datetime.now().isoformat()
        )
        ...
        # 从用户活跃列表移除（幂等操作，重复调用不会出错）
        status_data = await self.get_session_status(session_id)
        if status_data:
            user_id = status_data.get("user_id")
            if user_id:
                removed_count = await client.srem(f"user:{user_id}:sessions", session_id)
                if removed_count > 0:
                    logger.debug(f"✅ 已从活跃列表移除: session_id={session_id}, user_id={user_id}")
    ...
    async def update_heartbeat(self, session_id: str) -> None:
        ...
        # 更新心跳时间戳（60秒 TTL）
        await client.set(f"session:{session_id}:heartbeat", now, ex=60)
    ...
    async def buffer_event(
        ...
        # 方式2：分别传入字段（向后兼容）
            ...
            event = {"id": event_id, "type": event_type, "data": event_data, "timestamp": timestamp}
    ...
    async def get_events(
        ...
        # 读取所有缓冲的事件
        events_json = await client.lrange(f"session:{session_id}:events", 0, -1)
    ...
    async def stream_events(
        self, session_id: str, after_id: Optional[int] = None, timeout: int = 60
    ):
        """
        ...
        while True:
            ...
            # 读取新事件
            events = await self.get_events(session_id=session_id, after_id=last_id, limit=10)

            # 发送新事件
            for event in events:
                yield event
                event_seq = event.get(
                    "seq", event.get("id", 0) if isinstance(event.get("id"), int) else 0
                )
    ...
    async def subscribe_events(
        self, session_id: str, after_id: Optional[int] = None, timeout: int = 1800
    ):
        """
        ...
        if after_id is not None:
            backlog_events = await self.get_events(
                session_id=session_id, after_id=after_id, limit=1000
            )
        ...
        try:
            ...
            while True:
                ...
                if idle_seconds >= ping_interval:
                    ...
                    logger.debug(
                        f"💓 发送 ping 心跳: session_id={session_id}, idle={idle_seconds:.0f}s"
                    )
                ...
                if session_data.get("status") in ["completed", "failed", "stopped"]:
                    ...
                    final_events = await self.get_events(
                        session_id=session_id, after_id=last_id, limit=100
                    )
    ...
    async def set_stop_flag(self, session_id: str) -> None:
        ...
        # 设置停止标志（60秒 TTL，防止泄漏）
        await client.set(f"session:{session_id}:stop_flag", "1", ex=60)
...
def get_redis_manager(
    ...
    if _default_redis_manager is None:
        ...
        _default_redis_manager = RedisSessionManager(
            redis_host=host, redis_port=port, redis_db=redis_db, redis_password=password
        )
