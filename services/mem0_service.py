...
class Mem0Service:
    ...
    async def search(self, user_id: str, query: str, limit: int = 10) -> List[MemoryItem]:
        """
        搜索用户相关记忆
        ...
        try:
            pool = self._get_pool()
            memories = pool.search(user_id=user_id, query=query, limit=limit)
    ...
    async def search_with_rerank(
        self, user_id: str, query: str, limit: int = 5, candidate_limit: int = 30
    ) -> List[MemoryItem]:
        """
        搜索并重排序用户记忆（提升召回精度）
        ...
        try:
            # 1. 召回更多候选
            pool = self._get_pool()
            candidates = pool.search(user_id=user_id, query=query, limit=candidate_limit)
            ...
            reranked = await reranker.rerank(query=query, memories=candidates, top_k=limit)
    ...
    async def get_all(self, user_id: str, limit: int = 50) -> List[MemoryItem]:
        """
        获取用户所有记忆
    ...
    async def add(
        ...
        try:
            pool = self._get_pool()
            result = pool.add(user_id=user_id, messages=messages, metadata=metadata)

            memories_added = len(result.get("results", []))
            logger.info(f"➕ 添加记忆: user_id={user_id}, 新增={memories_added}")

            return MemoryAddResult(memories_added=memories_added, results=result.get("results", []))
    ...
    async def batch_update(
        self, since_hours: int = 24, max_concurrent: int = 5
    ) -> BatchUpdateResult:
        """
        批量更新所有用户记忆
        ...
        try:
            ...
            logger.info(f"🚀 触发批量更新: since={since_hours}h, max_concurrent={max_concurrent}")

            result = await batch_update_all_memories(
                since_hours=since_hours, max_concurrent=max_concurrent, service=service
            )
    ...
    async def update_user(self, user_id: str, since_hours: int = 24) -> UpdateResult:
        """
        更新单个用户的记忆
        ...
        try:
            ...
            result = await update_user_memories(
                user_id=user_id, since_hours=since_hours, service=service
            )
    ...
    async def health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            pool_info = {"type": type(pool).__name__, "available": True}

            return HealthCheckResult(service="mem0", status="healthy", pool=pool_info)

        except Mem0NotInstalledError as e:
            return HealthCheckResult(service="mem0", status="unavailable", error=str(e))
        except Exception as e:
            return HealthCheckResult(service="mem0", status="unhealthy", error=str(e))
