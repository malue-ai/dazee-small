...
class AgentRegistry:
    ...
    async def create_agent(
        ...
        Args:
            agent_id: Agent ID（将作为目录名，通常是自动生成的 UUID）
            name: Agent 名称（用户可读的显示名称）
        ...
        if not re.match(r"^[a-zA-Z][a-zA-Z0-9_-]*$", agent_id):
            raise ValueError(
                f"agent_id 格式不合法: {agent_id}，必须以字母开头，只能包含字母、数字、下划线、连字符"
            )
...
"""
            ...
            logger.info(f"✅ Agent '{agent_id}' ({display_name}) 创建成功")

            return {
                "agent_id": agent_id,
                "name": display_name,
                "path": str(agent_dir),
                "message": f"Agent '{display_name}' 创建成功",
            }
