...
class HealthProbeService:
    ...
    def __init__(
        ...
        """
        # 🆕 V10.3: 配置参数先保存，配置加载移到 _load_config() 异步方法
        self._init_interval = interval_seconds
        self._init_timeout = timeout_seconds
        self._init_profiles = profiles

        # 使用默认值初始化（会在 start() 中被异步加载的配置覆盖）
        self.enabled = True
        self.interval = interval_seconds if interval_seconds is not None else 120
        self.timeout = timeout_seconds if timeout_seconds is not None else 10.0
        self.profiles = (
            profiles
            if profiles is not None
            else ["main_agent", "intent_analyzer", "lead_agent", "worker_agent", "critic_agent"]
        )
    ...
    async def _load_config(self) -> None:
        ...
        self.interval = (
            self._init_interval
            if self._init_interval is not None
            else bg_config.get("interval_seconds", 120)
        )
        self.timeout = (
            self._init_timeout
            if self._init_timeout is not None
            else bg_config.get("timeout_seconds", 10.0)
        )
        self.profiles = (
            self._init_profiles
            if self._init_profiles is not None
            else bg_config.get(
                "profiles",
    ...
    async def probe_all(self) -> Dict[str, Dict[str, Any]]:
        ...
        for profile_name in self.profiles:
            try:
                ...
                if status == "healthy":
                    if switched:
                        logger.info(
                            f"   [{profile_name}] ✅ 健康 (已切换到备用: {selected.get('name', '未知')})"
                        )
                    else:
                        logger.info(
                            f"   [{profile_name}] ✅ 健康 (主密钥: {selected.get('name', '未知')})"
                        )
                elif status == "degraded":
                    logger.info(
                        f"   [{profile_name}] ⚠️ 降级 (备用密钥: {selected.get('name', '未知')})"
                    )
                else:
        ...
        # 汇总日志
        healthy_count = sum(
            1 for r in results.values() if r.get("status") in ("healthy", "degraded")
        )
    ...
    async def _get_or_create_llm_service(self, profile_name: str) -> Optional[Any]:
        ...
        try:
            ...
            profile = await get_llm_profile(profile_name)
