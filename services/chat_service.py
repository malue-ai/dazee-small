...
from services.chat_event_handler import ChatEventHandler
...
class ChatService:
    ...
    async def _run_agent(
        ...
        1. 保存用户消息
        2. 创建 Assistant 消息占位
        3. 调用 Agent.chat()，用 ChatEventHandler 处理事件
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ 保存消息失败: {str(e)}")
            ...
            # 确保消息被保存（即使没有 message_stop 事件）
            if not handler.is_finalized and handler.has_content():
                logger.warning(f"⚠️ 循环结束但消息未保存，执行补偿保存")
                await handler.finalize(agent)
