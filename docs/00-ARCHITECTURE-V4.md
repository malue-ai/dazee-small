...
│  │  ┌──────────────────────────────────────────────────────────────────────┐  ││
│  │  │                        ToolSelector                                   │  ││
│  │  │   • Schema 驱动优先（从 AgentSchema.tools 读取）                      │  ││
│  │  │   • 备用: 按 capability_tag 从 Registry 匹配                          │  ││
│  │  │   • 选择优先级：Schema > Plan > Intent                                │  ││
│  │  │   ⚠️ 注意：未使用 CapabilityRouter 的评分算法                         │  ││
│  │  └──────────────────────────────────────────────────────────────────────┘  ││
│  │  ┌──────────────────────────────────────────────────────────────────────┐  ││
│  │  │                        ToolExecutor                                   │  ││
│  │  │   • 动态加载工具实例（从 tools/ 目录）                                │  ││
...
class InvocationSelector:
    ...
    ⚠️ V4 状态：由 System Prompt 7 级优先级表指导 Claude 自主选择
    ...
    当前替代方案：
    • System Prompt 7 级优先级表指导 Claude 选择
    • 工具内部自行处理复杂调用逻辑
