...
│                                      ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ 2. Plan Creation (if needs_plan) 🆕 V4.6 修正                             │   │
│  │    plan_todo.create_plan(user_query)                                      │   │
│  │    内部流程：                                                              │   │
│  │    ├─ 只使用 capability_categories（10 种能力类别）                      │   │
│  │    │   ❌ 不注入具体工具列表（避免上下文过长）                            │   │
│  │    ├─ 匹配 task_type → 确定所需 capability                               │   │
│  │    └─ Claude + Extended Thinking → 生成 Plan（按能力类别分步骤）         │   │
│  │    → Plan { goal, steps[{ capability }], recommended_skill }             │   │
│  └───────────────────────────────────┬──────────────────────────────────────┘   │
│                                      │                                           │
│                                      ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ 3. Tool Selection（执行阶段，每个步骤动态选择）🆕 V4.6                     │   │
│  │    ToolSelector.select(required_capabilities)                             │   │
│  │                                                                           │   │
│  │    📚 Level 1/2 分层 + 实例工具合并：                                     │   │
│  │    ├─ Level 1: plan_todo, bash, request_human_confirmation（始终加载）   │   │
│  │    ├─ Level 2: 根据 task_type_mappings 按需加载                          │   │
│  │    └─ Instance: InstanceToolRegistry.get_tools_for_claude()              │   │
│  │         ├─ MCP 工具（instances/xxx/config.yaml）                         │   │
│  │         └─ REST API（提示词注入 + api_calling 工具）                      │   │
│  │    → tools_for_llm（合并后发送给 Claude）                                │   │
│  └───────────────────────────────────┬──────────────────────────────────────┘   │
│                                      │                                           │
│                                      ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
...
│  │        ┌─────────────────────────────────────────────────────────────┐    │   │
│  │        │ [Read]   ← Plan 状态 + 当前步骤的 capability                 │    │   │
│  │        │ [Reason] ← LLM Extended Thinking                             │    │   │
│  │        │ [Act]    ← LLM 选择具体工具（从 tools_for_llm 中）           │    │   │
│  │        │              │                                               │    │   │
│  │        │              ▼                                               │    │   │
│  │        │          ToolExecutor.execute(tool_name, input)             │    │   │
│  │        │              │                                               │    │   │
│  │        │              ├─ 全局工具 → _tool_instances[name].execute()  │    │   │
│  │        │              ├─ MCP 工具 → _tool_handlers[name](input)      │    │   │
│  │        │              │              → MCPClientWrapper.call_tool()  │    │   │
│  │        │              └─ Claude Skills → Anthropic 托管执行           │    │   │
│  │        │              │                                               │    │   │
│  │        │              ▼                                               │    │   │
│  │        │ [Observe] ← Tool Result                                      │    │   │
...
## 🔀 V4.6 Skills + Tools 整合

### 5.0 工具来源与配置（🆕 V4.6）

V4.6 支持四种工具来源，每种来源有不同的配置方式和执行机制：

| 来源 | 配置文件 | 管理组件 | 执行方式 | 面向角色 |
|------|----------|----------|----------|----------|
| **全局工具** | `config/capabilities.yaml` | `CapabilityRegistry` | `ToolExecutor._tool_instances` | 开发者 |
| **MCP 工具** | `instances/xxx/config.yaml` | `InstanceToolRegistry` | `ToolExecutor._tool_handlers` → `MCPClientWrapper` | 运营/FDE |
| **REST API** | `instances/xxx/api_desc/*.md` | 提示词注入 | 全局 `api_calling` 工具 | 运营/FDE |
| **Claude Skills** | `container.skills` | Anthropic 托管 | LLM 原生 | 开发者 |
...
│  │   ├─ capability_categories: 10 种能力类别（Plan 阶段使用）               │   │
│  │   └─ capabilities: 工具/Skills 详细定义                                  │   │
│  │                                                                          │   │
│  │   加载时机: 框架启动时（单例模式 get_capability_registry）               │   │
│  │                                                                          │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 实例配置（运营/FDE 维护）                                                │   │
│  │                                                                          │   │
│  │   instances/xxx/config.yaml                                              │   │
│  │   ├─ mcp_tools: MCP 服务器声明（server_url, auth_env）                   │   │
│  │   └─ apis: REST API 配置（base_url, auth, doc）                          │   │
│  │                                                                          │   │
│  │   instances/xxx/api_desc/*.md                                            │   │
│  │   └─ REST API 自然语言文档（注入提示词，LLM 阅读后调用 api_calling）     │   │
│  │                                                                          │   │
│  │   加载时机: create_agent_from_instance() 时                              │   │
│  │                                                                          │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
...
### 5.1 Claude Skills vs Tools 关系（V4.6 更新）
...
│      ▼                                                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ Plan 阶段：只使用能力类别（🆕 V4.6 修正）                                 │   │
│  │   plan_todo.create_plan() → capability_categories（10 种）               │   │
│  │   ❌ 不注入具体工具列表（避免上下文过长）                                 │   │
│  └───────────────────────────────────┬──────────────────────────────────────┘   │
│                                      │                                           │
│          ┌───────────────────────────┴───────────────────────┐                  │
...
if plan and plan.get("recommended_skill"):
    ...
    # ========== Tool 路径（V4.6 更新）==========
    # 1. 工具选择（Level 1/2 分层 + 实例工具合并）
    tools_for_llm = tool_selector.get_tools_for_llm(...)
    
    # 2. 🆕 合并实例级工具（MCP 工具）
    if instance_tool_registry:
        instance_tools = instance_tool_registry.get_tools_for_claude()
        tools_for_llm.extend(instance_tools)
    
    # 3. Claude 自主选择工具（包括 MCP 工具）
...
# ========== 工具执行路由（V4.6）==========
# ToolExecutor.execute(tool_name, tool_input) 内部路由：

async def execute(tool_name: str, tool_input: dict):
    # 1. MCP 工具 → 通过注册的处理器调用
    if tool_name in self._tool_handlers:
        return await self._tool_handlers[tool_name](tool_input)
        # _tool_handlers[name] 在 instance_loader 中注册
        # 实际调用: MCPClientWrapper.call_tool(original_name, tool_input)
    
    # 2. 全局工具 → 通过实例调用
    if tool_name in self._tool_instances:
        return await self._tool_instances[tool_name].execute(**tool_input)
    
    # 3. Claude Skills → Anthropic 托管执行（不经过 ToolExecutor）
```
...
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │ 1. 全局工具（CapabilityRegistry）                                        │    │
│  │    配置文件: config/capabilities.yaml                                    │    │
│  │    加载时机: 框架启动时一次性加载（单例模式）                             │    │
│  │    管理组件: get_capability_registry() 返回单例实例                      │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │ 2. 实例工具（InstanceToolRegistry）                                      │    │
│  │    配置文件: instances/xxx/config.yaml                                   │    │
│  │    加载时机: 实例启动时加载                                              │    │
│  │    管理组件: InstanceToolRegistry                                        │    │
│  │    工具类型:                                                             │    │
│  │      • MCP 工具: mcp_tools[] → MCPClientWrapper                         │    │
│  │      • REST API: apis[] → 提示词注入 + api_calling                      │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
│                                                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │ 3. Claude Skills（Anthropic 托管）                                       │    │
│  │    配置方式: container.skills[]                                          │    │
│  │    执行环境: Anthropic 服务器                                            │    │
│  │    激活条件: Plan 阶段 recommended_skill 匹配                            │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
│                                                                                  │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
```
启动阶段:
  instance_loader.create_agent_from_instance()
      │
      ├─ get_mcp_client(server_url) → 获取/创建客户端（带缓存）
      ├─ MCPClientWrapper.connect() → 连接 MCP 服务器
      ├─ MCPClientWrapper.list_tools() → 发现可用工具
      ├─ InstanceToolRegistry.register_mcp_tool() → 注册元信息
      └─ ToolExecutor.register_handler() → 注册执行处理器

执行阶段:
  ToolExecutor.execute(tool_name, input)
      │
      ├─ _tool_handlers[name]? → handler(input) → MCPClientWrapper.call_tool()
      └─ 否则 → 全局工具实例执行

关闭阶段:
  clear_mcp_client_cache() → 断开所有 MCP 连接
```
...
apis:
  - name: weather_api
    ...
    auth:
      type: bearer
      env: WEATHER_API_KEY
    doc: weather  # → api_desc/weather.md
```

执行流程：
1. `api_desc/weather.md` 内容注入到 System Prompt
2. LLM 阅读文档理解 API 用法
3. LLM 调用全局 `api_calling` 工具执行 HTTP 请求
...
| **全局 Tools** | 标准 tool_use 调用的工具（全局配置） | `tools/` + `config/capabilities.yaml` |
| **MCP 工具** | 通过 MCP 协议连接的外部服务工具（🆕） | `instances/xxx/config.yaml` → `mcp_tools` |
| **REST API** | 通过提示词描述 + api_calling 调用（🆕） | `instances/xxx/api_desc/*.md` |
| **连接层** | 任意外部能力接入（不仅是 MCP） | Tools + REST API + E2B + MCP + ... |
| **DIRECT** | 标准 tool_use 调用方式 | 包括 E2B、exa_search、MCP 工具等 |
| **CODE_EXECUTION** | Anthropic code_execution | 仅指 Anthropic 托管的代码执行环境 |
| **CapabilityRegistry** | 全局能力注册表（单例）（🆕） | `core/tool/capability/registry.py` |
| **InstanceToolRegistry** | 实例级工具注册表（🆕） | `core/tool/instance_registry.py` |
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│  User Query: "帮我生成用户管理系统的 flowchart"                                  │
│      │                                                                           │
│      ▼                                                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ 1. Plan 阶段（能力类别级别）                                              │   │
│  │    plan_todo.create_plan(user_query)                                      │   │
│  │                                                                           │   │
│  │    可用能力类别（capability_categories，共 11 种）：                      │   │
│  │                                                                           │   │
│  │    📌 用户意图类别（Plan 主要使用）：                                     │   │
...
│  │    • code_execution    - 代码执行                                         │   │
│  │    • code_sandbox      - 沙箱运行                                         │   │
│  │    • api_calling       - API 调用（⚠️ 属于实现手段，非用户意图）          │   │
│  │                                                                           │   │
│  │    ❌ 不注入具体工具列表（100+ 工具 → 上下文过长）                        │   │
│  │    ✅ Plan 输出用户意图类别，工具选择阶段再映射到具体工具                 │   │
│  │                                                                           │   │
│  │    → Plan: { steps: [{ capability: "document_creation" }] }              │   │
│  │      （"生成 flowchart" → 归类为文档创建意图）                            │   │
│  └───────────────────────────────────┬──────────────────────────────────────┘   │
│                                      │                                           │
│                                      ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ 2. 工具选择阶段（具体工具级别）                                           │   │
│  │    ToolSelector.select(required_capabilities=["document_creation"])       │   │
│  │                                                                           │   │
│  │    Level 1 工具（始终加载）：                                             │   │
│  │    • plan_todo         • bash                • request_human_confirmation│   │
│  │                                                                           │   │
│  │    Level 2 工具（按需加载，根据 task_type_mappings）：                    │   │
│  │    • api_calling       • knowledge_search    • exa_search                │   │
│  │                                                                           │   │
│  │    实例工具（从 InstanceToolRegistry 合并）：                             │   │
│  │    • dify_flowchart（MCP 工具）                                          │   │
│  │                                                                           │   │
│  │    → tools_for_llm: [plan_todo, api_calling, dify_flowchart, ...]        │   │
│  └───────────────────────────────────┬──────────────────────────────────────┘   │
│                                      │                                           │
│                                      ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ 3. LLM 自主选择（智能级别）                                               │   │
│  │    Claude Extended Thinking + tool_use                                    │   │
│  │                                                                           │   │
│  │    LLM 分析 tools_for_llm 中的工具描述：                                 │   │
│  │    • dify_flowchart: "根据用户输入生成 flowchart 格式富文本"             │   │
│  │    • api_calling: "通用 HTTP API 调用"                                   │   │
│  │                                                                           │   │
│  │    → 选择 dify_flowchart（更精准匹配用户需求）                           │   │
│  └───────────────────────────────────┬──────────────────────────────────────┘   │
│                                      │                                           │
│                                      ▼                                           │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │ 4. 统一执行路由                                                           │   │
│  │    ToolExecutor.execute("dify_flowchart", input)                          │   │
│  │                                                                           │   │
│  │    路由决策：                                                             │   │
│  │    ├─ _tool_handlers["dify_flowchart"]? ✓                                │   │
│  │    │     → MCPClientWrapper.call_tool("Ontology_TextToChart_zen0")       │   │
│  │    ├─ _tool_instances["..."]?                                            │   │
│  │    │     → 全局工具实例执行                                              │   │
│  │    └─ CLAUDE_SERVER_TOOLS?                                               │   │
│  │          → Anthropic 托管执行                                            │   │
...
**设计原则**：`capability_categories` 应以**用户意图**为主，**实现手段**为辅。

| 类型 | 类别 | 说明 |
|------|------|------|
| 用户意图（What） | `knowledge_base`, `web_search`, `ppt_generation`, `document_creation`, `data_analysis`, `app_generation`, `task_planning` | Plan 阶段主要使用，描述用户想要完成什么 |
| 实现手段（How） | `file_operations`, `code_execution`, `code_sandbox`, `api_calling` | 工具选择阶段使用，描述如何实现目标 |

**⚠️ 当前设计的局限**：
- `api_calling` 属于实现手段，但被混在能力类别中
- Plan 输出时应优先使用用户意图类别（如 `document_creation`），而非实现手段类别

**建议的 Plan 流程**：
```
用户: "生成用户管理系统的 flowchart"
  ↓
Plan 分析意图: "生成图表/文档" → capability: document_creation
  ↓
工具选择: document_creation → 匹配实例工具 dify_flowchart（MCP）
  ↓
执行: ToolExecutor → MCPClientWrapper.call_tool()
```
...
| 工具类型 | 配置来源 | 注册组件 | 执行路径 |
|----------|----------|----------|----------|
| 全局 TOOL | `config/capabilities.yaml` | `CapabilityRegistry` | `ToolExecutor._tool_instances` |
| 全局 SKILL | `config/capabilities.yaml` | `CapabilityRegistry` | `container.skills` |
| MCP 工具 | `instances/xxx/config.yaml` | `InstanceToolRegistry` | `ToolExecutor._tool_handlers` |
| REST API | `instances/xxx/api_desc/*.md` | System Prompt 注入 | `api_calling` 工具 |
...
core/tool/
├── capability/
│   ├── registry.py          # CapabilityRegistry（单例）
│   ├── router.py             # 能力路由（预留）
│   └── invocation.py         # InvocationSelector
├── instance_registry.py      # InstanceToolRegistry
├── executor.py               # ToolExecutor（统一执行）
└── selector.py               # ToolSelector（Level 1/2）

scripts/
└── instance_loader.py        # 实例加载 + MCP 注册

services/
└── mcp_client.py             # MCPClientWrapper + 缓存
```
...
instances/
└── my_agent/
    ├── config.yaml           # 实例配置（MCP 工具、REST API、Agent 参数）
    ├── prompt.md             # 实例提示词（角色定义、能力描述）
    ├── .env                  # 环境变量（API 密钥，不提交到版本库）
    ├── env.example           # 环境变量模板
    ├── api_desc/             # REST API 文档
    │   └── *.md              # 各 API 的使用说明
    └── skills/               # Claude Skills（可选）
        ├── skill_registry.yaml
        └── */SKILL.md
```
...
mcp_tools:
  ...
  - name: dify_flowchart
    ...
    server_name: "dify"          # 用于工具命名空间前缀
    auth_type: "bearer"          # none / bearer / api_key
    auth_env: "DIFY_API_KEY"     # 从 .env 读取的环境变量名
    description: "生成 flowchart 格式的富文本"
...
apis:
  - name: weather_api
    ...
    auth:
      ...
      header: Authorization     # 认证头名称
      env: WEATHER_API_KEY      # 环境变量名
    doc: weather                # 指向 api_desc/weather.md
    description: "天气查询 API"
```

```markdown
<!-- instances/my_agent/api_desc/weather.md -->
# Weather API
...
| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| MCP 连接失败 | URL 错误或服务未启动 | 检查 `server_url` 和服务状态 |
| 认证失败 | 环境变量未设置 | 检查 `.env` 文件和 `auth_env` 配置 |
| 工具未发现 | MCP 服务无工具暴露 | 使用 `client.list_tools()` 调试 |
| REST API 调用失败 | 文档描述与实际 API 不符 | 更新 `api_desc/*.md` 文档 |
| 异步错误 | MCP 连接未正确关闭 | 确保程序退出时调用 `clear_mcp_client_cache()` |
...
# 运行实例（交互模式）
python scripts/run_agent.py --instance my_agent

# 运行实例（单次查询）
python scripts/run_agent.py --instance my_agent --query "生成用户管理系统 flowchart"
