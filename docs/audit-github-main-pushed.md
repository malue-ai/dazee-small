...
- **evaluation/ 目录：保留**。评测框架与 suites 可留在 GitHub，但**必须脱敏**：
  - 所有真实服务 URL（如 `https://agent.dazee.ai`）→ 改为占位符（如 `https://your-cloud-agent.example.com`）。
...
TARGET_USER_OPEN_ID = "ou_5e6863a9588e4c91d5e7512519b2d7dd"
```
...
```yaml
  Name: sk-1650eb369f834d77a4148898ce04a6c6   # 疑似 DASHSCOPE_API_KEY
```
