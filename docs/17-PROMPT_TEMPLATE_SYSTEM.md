...
│   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  │
│                                                                              │
│   • Session 生命周期                   • System Prompt 追加                   │
│     - session_start                      - prompt_append（事件类型）         │
│     - session_end                                                            │
│                                                                              │
│   • Conversation 生命周期              触发条件（由事件驱动）：               │
│     - conversation_start                 - file_uploaded 事件                │
│     - conversation_delta                 - rag_completed 事件                │
│     - conversation_stop                  - task_detected 事件                │
│                                          - tool_selected 事件                │
│   • Message 生命周期                     - context_injected 事件             │
│     - message_start                                                          │
│     - message_delta                                                          │
│     - message_stop                                                           │
│                                                                              │
...
```python
# Prompt 相关事件（扩展 ConversationEventManager）

class PromptEventType(Enum):
    """Prompt 相关事件类型"""
    
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # 触发追加的事件（由 Agent 运行过程产生）
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    FILE_UPLOADED = "file_uploaded"          # 文件上传
    RAG_COMPLETED = "rag_completed"          # RAG 检索完成
    TASK_DETECTED = "task_detected"          # 任务类型识别
    TOOL_SELECTED = "tool_selected"          # 工具选择
    CONTEXT_INJECTED = "context_injected"    # 上下文注入（前端变量等）
    
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # 追加完成的事件（供前端/日志使用）
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    PROMPT_APPEND = "prompt_append"          # Prompt 片段已追加
```
...
# → PromptManager 订阅此事件，自动追加文件处理规范

# 2. RAG 检索完成 → 触发 rag_completed 事件
await event_manager.emit_custom(
    session_id=session_id,
...
# → PromptManager 订阅此事件，自动追加 RAG 上下文

# 3. 任务类型识别 → 触发 task_detected 事件
await event_manager.emit_custom(
    session_id=session_id,
...
# → PromptManager 订阅此事件，自动追加 PPT 规范

# 4. 追加完成后，发送 prompt_append 事件（供前端/日志使用）
...
class PromptManager:
    ...
    def __init__(self):
        ...
        self._append_rules: List[PromptAppendRule] = [
            ...
            # task_detected → 追加任务规范（根据 task_type）
            PromptAppendRule(
                event_type="task_detected",
                fragment_id="ppt_rules",
                condition=lambda data: data.get("task_type") == "ppt_generation"
            ),
            PromptAppendRule(
                event_type="task_detected",
                fragment_id="excel_rules",
                condition=lambda data: data.get("task_type") == "excel_generation"
            ),
...
│   │   │  task_detected     → 追加 ppt_rules.md / excel_rules.md     │  │   │
