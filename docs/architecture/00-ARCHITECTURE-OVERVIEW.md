...
│  │  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌─────────────┐│  │
│  │  │ messages  │  │ plan.json │  │  todo.md  │  │🆕 sandbox   ││  │
│  │  │           │  │           │  │           │  │   sessions  ││  │
│  │  └───────────┘  └─────↑─────┘  └─────↑─────┘  └─────↑───────┘│  │
│  │                       │ CRUD         │               │ 会话管理│  │
│  │  ┌────────────────────┴──────────────┴───────────────┴────────┐│  │
...
│  │  │  🆕 • manage_sandbox_session → E2B会话生命周期            ││  │
│  │  └───────────────────────────────────────────────────────────┘│  │
│  └───────────────────────────────────────────────────────────────┘  │
│                                                                      │
│  ┌──────────────────────────────────────────────────────────────┐  │
...
│  │  │    - code_execution (Claude内置)                          │  │
│  │  │    🆕 - code_sandbox (E2B沙箱) ← 新能力                   │  │
│  │  │  • capabilities (工具定义)                                │  │
│  │  └────────┬────────┘                                          │  │
│  │           │ 加载配置                                           │  │
│  │           ▼                                                    │  │
...
│  │    🆕 • Code Execution (代码执行)                            │  │
│  │      - Claude内置: 简单计算/JSON生成 (<10s, 无第三方包)      │  │
│  │      - E2B沙箱: 网络访问/第三方包/文件持久化/长时运行        │  │
│  │    • Programmatic Tool Calling (多工具编排>2，优先E2B)       │  │
...
│  │  🆕 E2B 工具集:                                                │  │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐        │  │
│  │  │e2b_python    │  │e2b_template  │  │e2b_vibe      │        │  │
│  │  │_sandbox      │  │_manager      │  │_coding       │        │  │
│  │  │━━━━━━━━━━━━━━│  │━━━━━━━━━━━━━━│  │━━━━━━━━━━━━━━│        │  │
│  │  │代码执行      │  │模板管理      │  │完整应用生成  │        │  │
│  │  │流式输出      │  │环境配置      │  │Streamlit/GUI │        │  │
│  │  │包管理        │  │              │  │              │        │  │
│  │  └──────┬───────┘  └──────────────┘  └──────┬───────┘        │  │
│  │         │                                    │                 │  │
│  │         └────────────────┬───────────────────┘                 │  │
...
│                             ▼                                        │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  🆕 E2B Sandbox Service (外部服务)                            │  │
│  │  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  │  │
│  │  • Python沙箱环境（隔离、安全）                               │  │
│  │  • 完整网络访问（requests, httpx）                            │  │
│  │  • 自动pip安装（pandas, numpy等）                             │  │
│  │  • 文件系统持久化（2GB存储）                                  │  │
│  │  • 长时间运行（最长30分钟）                                    │  │
│  │  • 会话复用（同session内共享环境）                            │  │
│  │  • 流式输出（实时stdout/stderr）                              │  │
│  │  ┌────────────┐  ┌────────────┐  ┌────────────┐              │  │
│  │  │ Python环境 │  │ 文件系统   │  │ 网络访问   │              │  │
│  │  │ pip packages│  │ workspace/ │  │ HTTP/HTTPS │              │  │
│  │  └────────────┘  └────────────┘  └────────────┘              │  │
│  └───────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
        ...
        ├───────────────────────────────────────────────────────────┤
        │  🆕 组装内容（注入到 Sonnet 的 system 参数）：             │
        │                                                            │
        │  ① Base Prompt (universal_agent_prompt.py)                │
        │     • RVR 循环、状态管理、执行流程等核心规则               │
        │                    ＋                                      │
        │  ② Capability Categories (capabilities.yaml → Registry)   │
        │  ┌─────────────────────────────────────────────────────┐  │
        ...
        │     • 优先级判断（由 Sonnet 自主决定）                     │
        │                    ＋                                      │
        │  ④ Skills Metadata (SkillsManager)                        │
        │  ┌─────────────────────────────────────────────────────┐  │
        │  │ ## 📚 Available Skills                              │  │
        │  │                                                      │  │
        │  │ ### slidespeak-generator                            │  │
        │  │ 描述: 专业PPT设计指南和API使用说明                  │  │
        │  │ 触发词: [专业PPT, 产品演示, 业务汇报...]           │  │
        │  │ 引用工具: [slidespeak_render, api_calling]         │  │
        │  │ 路径: skills/library/slidespeak-generator/SKILL.md │  │
        │  │ 💡 使用: bash cat <path> 获取详细指导              │  │
        │  └─────────────────────────────────────────────────────┘  │
        │                                                            │
        │  → 完整 System Prompt 准备就绪                            │
        │  → Sonnet 知道: 有哪些能力分类、有哪些 Skills 可用         │
        │  → 决策权: Sonnet 根据任务自主判断是否加载 Skill           │
        │                                                            │
        │  📌 注意: Tool Definitions 通过 Claude API 的 tools 参数  │
        │          传递，不在 System Prompt 中                       │
        └───────────────────────┬───────────────────────────────────┘
                                │
                                ▼
        ┌───────────────────────────────────────────────────────────┐
