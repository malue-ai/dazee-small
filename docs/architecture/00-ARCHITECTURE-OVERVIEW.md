...
> **最后更新**: 2026-01-15  
> **当前版本**: V7.1 - 多智能体生产就绪 + Anthropic 启发优化  
> **历史版本**: 已归档至 [`archived/`](./archived/) 目录  
> **架构状态**: 多智能体池化完成，Prompts Engineering 核心就绪
...
```
V5.0 → V5.1 → V6.1 → V6.2 → V6.3 → V7.0 → V7.1
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
实例缓存  评估体系  场景化   配置优先级  上下文   架构重构  多智能体
LLM驱动  共享层    提示词   Schema兜底  三层防护  单/多独立  生产就绪
         剥离     分解     配置合并              生产就绪  Anthropic启发
```
...
| ChatService 集成 AgentRouter | ✅ | `services/chat_service.py` |
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   用户请求 → ChatService → AgentRouter（路由层）                         │
│                                ↓                                         │
│                         ┌─────┴─────┐                                    │
│                         ↓           ↓                                    │
│                  ┌──────────┐  ┌──────────────┐                          │
...
│  │  │ • exa_search  │ │ • pptx        │ │ • api.py      │ │ • token_count │    │  │
│  │  │ • knowledge   │ │ • slidespeak  │ │ • file.py     │ │ • validators  │    │  │
│  │  └───────────────┘ └───────────────┘ └───────────────┘ └───────────────┘    │  │
│  │  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐                      │  │
...
│       │     • needs_plan: true                                              │
│       │                                                                      │
│       └─→ ComplexityScorer.score()     ← 评分 1-10                          │
│                ↓                                                             │
│            RoutingDecision:                                                  │
...
└─────────────────────────────────────────────────────────────────────────────┘
    ...
    ↓ (根据 routing decision)
┌─────────────────────────────────────────────────────────────────────────────┐
│ Phase 2: Agent 执行 (独立框架)                                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌─────────────────────┐              ┌─────────────────────┐               │
│  │   SimpleAgent       │      OR      │ MultiAgentOrchest.  │               │
│  │   (complexity ≤ 5)  │              │ (complexity > 5)    │               │
│  │                     │              │                     │               │
│  │   RVR 循环:         │              │   DAG 执行:         │               │
│  │   1. Read plan      │              │   1. 任务分解       │               │
│  │   2. Reason (LLM)   │              │   2. Worker 分配    │               │
│  │   3. Act (Tool)     │              │   3. 并行执行       │               │
│  │   4. Validate       │              │   4. 结果聚合       │               │
│  │   5. Write plan     │              │                     │               │
│  └─────────────────────┘              └─────────────────────┘               │
...
│ Phase 3: 流式响应                                                            │
├─────────────────────────────────────────────────────────────────────────────┤
│  SSE Events / gRPC Stream:                                                   │
│    • message_start                                                           │
│    • content_delta                                                           │
│    • tool_use                                                                │
│    • context_usage_update (V6.3)                                            │
│    • message_end                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 重试策略 | ✅ 一致 | `infra/resilience/retry.py` | 指数退避 |
...
| 版本 | 日期 | 核心变化 |
|------|------|---------|
| **V7.1** | 2026-01-15 | **当前版本**：多智能体生产就绪 + Anthropic 启发优化（池化、Prompts Engineering、成本预算、检查点恢复） |
...
| 文档 | 说明 |
|------|------|
| [00-ARCHITECTURE-OVERVIEW.md](./00-ARCHITECTURE-OVERVIEW.md) | V7.1 架构总览（本文档） |
