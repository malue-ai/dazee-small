...
│  │  │  • tools.py      │                     │  • chat_servicer │               │  │
│  │  │  • knowledge.py  │                     │  • session_servicer │            │  │
│  │  │  • files.py ...  │                     │  • health_servicer  │            │  │
│  │  └────────┬─────────┘                     └────────┬─────────┘               │  │
│  │           └────────────────────┬────────────────────┘                         │  │
│  └────────────────────────────────┼─────────────────────────────────────────────┘  │
│                                   ↓                                                 │
...
│  │  │ • Adapters    │ │ • TokenBudget  │ │ • ResultsWriter │ │               │    │  │
│  │  └───────────────┘ └────────────────┘ └───────────────┘ └───────────────┘    │  │
│  │  ┌──────────────────────┐ ┌───────────────┐ ┌───────────────┐                │  │
...
│              • complexity_score: float (优先来自 intent.complexity_score)   │
│                                                                              │
│  if agent_type == "multi": TokenBudget.check_budget()                        │
│     ├─→ allowed=false → 降级 single（预算限制）                               │
│     └─→ allowed=true  → 继续 multi（可返回 budget_warning）                   │
│                                                                              │
...
│ 阶段 1: 路由决策（AgentRouter）                                               │
├──────────────────────────────────────────────────────────────────────────────┤
│  ChatService → AgentRouter.route(user_query, conversation_history)           │
│    ↓                                                                          │
│  IntentAnalyzer.analyze() (使用 Haiku)                                        │
│    ↓                                                                          │
│  IntentResult(complexity_score=8.5)  # 高复杂度                               │
│    ↓                                                                          │
│  if complexity_score >= threshold (默认 5.0):                                 │
│    RoutingDecision(agent_type="multi")                                        │
│  else:                                                                        │
│    RoutingDecision(agent_type="single") → SimpleAgent                         │
│                                                                              │
│  if agent_type == "multi": TokenBudget.check_budget()                        │
│    ├─→ allowed=false → agent_type="single"                                   │
│    └─→ allowed=true  → 保持 multi（可返回 budget_warning）                   │
└──────────────────────────────────────────────────────────────────────────────┘
   │
   ↓ (agent_type="multi")
┌──────────────────────────────────────────────────────────────────────────────┐
...
├──────────────────────────────────────────────────────────────────────────────┤
│  MultiAgentOrchestrator.execute(intent, messages, session_id, message_id)    │
...
└──────────────────────────────────────────────────────────────────────────────┘
   ...
   ↓
┌──────────────────────────────────────────────────────────────────────────────┐
│ 阶段 3: 任务分解（LeadAgent - 使用 Opus）                                     │
├──────────────────────────────────────────────────────────────────────────────┤
│  LeadAgent.decompose_task(user_query, conversation_history, intent_info)     │
│    ↓                                                                          │
│  LLM.create_message_async(Opus) ← 使用任务分解 Prompt                         │
│    ↓                                                                          │
│  TaskDecompositionPlan:                                                      │
│    ├─→ subtasks: List[SubTask]                                               │
│    │     • id: "sub-1", description: "收集数据"                               │
│    │     • id: "sub-2", description: "分析数据", depends_on: ["sub-1"]        │
│    │     • id: "sub-3", description: "生成图表", depends_on: ["sub-2"]        │
│    │     • id: "sub-4", description: "撰写报告", depends_on: ["sub-3"]        │
│    ├─→ execution_mode: "parallel" | "sequential" | "hierarchical"            │
│    ├─→ depends_on: SubTask.depends_on  # 依赖关系                             │
...
└──────────────────────────────────────────────────────────────────────────────┘
   ...
   ↓
┌──────────────────────────────────────────────────────────────────────────────┐
│ 阶段 4: 子任务执行（SubAgents - 使用 Sonnet）                                 │
├──────────────────────────────────────────────────────────────────────────────┤
│  根据 config.mode 执行（plan.execution_mode 为建议）:                         │
│                                                                               │
│  ┌──────────────────────────────────────────────────────────────────────┐    │
│  │ 并行模式（无依赖任务同时执行）                                       │    │
│  │                                                                      │    │
...
│  │     ├─→ LLM.create_message_async(Sonnet)                             │    │
│  │     └─→ 返回 SubagentResult                                          │    │
│  │          ↓                                                           │    │
│  │   ┌──────────────────────────────────────────────────┐              │    │
│  │   │ 关键检查点（每个子任务完成后）                    │              │    │
...
│  │   └──────────────────────────────────────────────────┘              │    │
│  │          ↓                                                           │    │
│  │   SubAgent-2 (sub-2: "分析数据", depends_on sub-1)                   │    │
│  │     ├─→ 等待 sub-1 完成                                              │    │
...
│  │   ... 继续执行 sub-3, sub-4 ...                                      │    │
│  │                                                                      │    │
│  │   emit agent_start / agent_end（层级模式包含 sub_agent_start/end）   │    │
│  └──────────────────────────────────────────────────────────────────────┘    │
│                                                                               │
│  ⚡ 优化策略:                                                                  │
...
├──────────────────────────────────────────────────────────────────────────────┤
│  CriticAgent.critique(executor_output, plan_step, success_criteria)          │
│    ↓                                                                          │
│  LLM.create_message_async(Sonnet) ← 使用 Critic Prompt                        │
│    ↓                                                                          │
│  CriticResult:                                                                │
...
│  ┌────────────────────────────────────────────┐                              │
│  │ if pass: 完成当前步骤                      │                              │
│  │ if retry: 注入建议并重试（低信心需人工）   │                              │
│  │ if replan: _trigger_replan() 调整计划      │                              │
│  │ if ask_human: 标记 needs_human_review      │                              │
│  └────────────────────────────────────────────┘                              │
│                                                                               │
│  结果写入 plan_step.metadata（供追踪与审计）                                 │
└──────────────────────────────────────────────────────────────────────────────┘
   │
   ↓
┌──────────────────────────────────────────────────────────────────────────────┐
...
| **层级模式** | 主从协作/子任务分发 | 上下文隔离、可扩展 | 设计与调试更复杂 |
