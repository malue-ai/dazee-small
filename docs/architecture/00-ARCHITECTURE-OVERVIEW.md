...
DEFAULT_AGENT_SCHEMA = AgentSchema(
    ...
    intent_analyzer=IntentAnalyzerConfig(
        enabled=True,
        use_llm=True,
        task_types=["question_answering", "data_analysis", 
                    "content_generation", "code_execution", ...],
    ),
    ...
    reasoning="高质量默认配置：适应大多数场景，平衡能力和稳定性",
)
```
...
```python
# core/prompt/framework_rules.py
...
- 减少 LLM 理解偏差
- 明确每个复杂度级别的内容边界
- 避免重复信息（如意图识别已由 IntentAnalyzer 完成）
...
| 配置字段 | config.yaml | LLM 推断 | DEFAULT_AGENT_SCHEMA | 最终值 |
|---------|-------------|---------|---------------------|-------|
| model | `claude-sonnet-4-5` | - | `claude-sonnet-4-5` | **claude-sonnet-4-5** (config) |
| max_turns | `20` | - | `15` | **20** (config) |
| plan_manager.max_steps | - | `10` | `15` | **10** (LLM) |
| output_formatter.format | - | - | `markdown` | **markdown** (default) |
