...
# 创建异步引擎（PostgreSQL）
# 连接池配置说明：
# - pool_size: 常驻连接数（默认 10，适合中等负载，生产环境建议 20-30）
# - max_overflow: 允许额外创建的连接数（高峰期使用，默认 20）
...
engine = create_async_engine(
    ...
    pool_size=int(os.getenv("DB_POOL_SIZE", "10")),      # 优化：5 → 10
    max_overflow=int(os.getenv("DB_MAX_OVERFLOW", "20")), # 优化：10 → 20
