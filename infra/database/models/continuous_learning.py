...
class PlaybookRecord(Base):
    ...
    trigger: Mapped[dict] = mapped_column(JSONType, default={}, nullable=False)
    
    # 执行策略（JSONB）
    # {"execution_strategy": "", "suggested_tools": [], "max_turns": 0, "planning_depth": ""}
    strategy: Mapped[dict] = mapped_column(JSONType, default={}, nullable=False)
    
    # 工具序列（JSONB）
    # [{"tool": "", "purpose": "", "reward": 0.0}]
    tool_sequence: Mapped[list] = mapped_column(JSONType, default=[], nullable=False)
    
    # 质量指标（JSONB）
    # {"avg_reward": 0.0, "success_rate": 0.0, "avg_turns": 0.0}
    quality_metrics: Mapped[dict] = mapped_column(JSONType, default={}, nullable=False)
...
class IntentCacheRecord(Base):
    ...
    # 意图结果（JSONB）
    intent_result: Mapped[dict] = mapped_column(JSONType, nullable=False)
