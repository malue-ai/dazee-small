...
from fastapi import APIRouter, HTTPException, UploadFile, File, Form, BackgroundTasks, Query, status
...
async def import_from_files(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"file_ids 格式错误: {str(e)}"
            )
        
        if len(file_id_list) > 20:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="批量导入最多支持 20 个文件"
            )
        ...
        async def import_single_file(file_id: str, index: int):
            ...
            try:
                ...
                from services.file_service import get_file_service, FileNotFoundError as FileNotFoundErr
                ...
                logger.info(f"✅ [{index+1}/{len(file_id_list)}] {file_record.get('filename')} 导入成功")
                return {
                    "success": True,
                ...
                logger.error(f"❌ [{index+1}/{len(file_id_list)}] 导入文件 {file_id} 失败: {str(e)}")
                return {
                    "success": False,
                    "filename": file_id,
                    "error": str(e)
...
async def download_document(
    ...
    try:
        ...
        if source in ["auto", "ragie"]:
            ...
            logger.info(f"✅ 从 Ragie 下载: document_id={document_id}, size={len(file_bytes)} bytes")
