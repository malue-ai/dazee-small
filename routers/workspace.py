...
# 注：已移除本地文件系统模式，仅使用沙盒
from services.sandbox_service import FileInfo as SandboxFileInfo
...
async def get_projects(conversation_id: str):
    ...
    try:
        ...
        if not sandbox_info or sandbox_info.status == "none":
            # 沙盒不存在时返回空列表
            return ProjectListResponse(conversation_id=conversation_id, projects=[])
...
"""
        ...
        if result.get("success") and result.get("stdout"):
            ...
            # 检测 Node.js 项目
            if (
                "package.json" in output
                and "NOT_FOUND" not in output.split("=== PACKAGE_JSON ===")[1].split("===")[0]
            ):
            ...
            # 检测 Python 项目
                ...
                req_section = (
                    output.split("=== REQUIREMENTS ===")[1].split("===")[0]
                    if "=== REQUIREMENTS ===" in output
                    else ""
                )
                pyproject_section = (
                    output.split("=== PYPROJECT ===")[1] if "=== PYPROJECT ===" in output else ""
                )
        ...
        return ProjectListResponse(conversation_id=conversation_id, projects=projects)
...
async def get_sandbox_status(conversation_id: str):
    ...
    try:
        ...
        if not info:
            return SandboxStatusResponse(conversation_id=conversation_id, status="none")
...
async def init_sandbox(conversation_id: str, request: SandboxInitRequest):
    """
    初始化沙盒
    ...
    try:
        ...
        info = await service.get_or_create_sandbox(
            conversation_id=conversation_id, user_id=request.user_id, stack=request.stack
        )
...
async def run_command(conversation_id: str, request: CommandRequest):
    """
    在沙盒中执行命令
    ...
    try:
        ...
        result = await service.run_command(
            conversation_id=conversation_id, command=request.command, timeout=request.timeout
        )
...
async def list_files(
    ...
    try:
        ...
        try:
            ...
            return FileListResponse(conversation_id=conversation_id, files=files, source="sandbox")
        except SandboxNotFoundError:
            # 沙盒不存在时返回空列表（而不是 404）
            logger.info(f"沙盒不存在，返回空列表: {conversation_id}")
            return FileListResponse(conversation_id=conversation_id, files=[], source="sandbox")
...
async def upload_file(
    ...
    try:
        ...
        return UploadResponse(success=True, path=full_path, size=result["size"])
...
async def write_file(
    conversation_id: str, path: str, content: str = Query(..., description="文件内容")
):
    """
    写入文件内容（用于编辑器保存，沙盒模式）
    ...
    try:
        ...
        return UploadResponse(success=True, path=result["path"], size=result["size"])
...
async def delete_file(conversation_id: str, path: str):
    ...
    try:
        ...
        return DeleteResponse(success=True, path=path, message="删除成功")
