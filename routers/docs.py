...
from models.docs import DocContentResponse, DocsStructureResponse
from services.docs_service import get_doc_content, get_docs_structure
...
async def api_get_doc_content(doc_path: str) -> DocContentResponse:
    ...
    if content is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"文档不存在: {doc_path}")
