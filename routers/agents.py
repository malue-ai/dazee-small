...
async def create_agent(request: AgentCreateRequest):
    ...
    try:
        # 生成 agent_id（如果未提供则自动生成 UUID）
        agent_id = request.agent_id
        if not agent_id:
            # 使用标准 UUID 格式（PostgreSQL 原生支持）
        ...
        logger.info(f"✅ 创建 Agent: {agent_id} (name={request.name})")
...
async def update_agent(agent_id: str, request: AgentUpdateRequest):
    ...
    try:
        ...
        if request.llm is not None:
            update_kwargs["llm"] = {
                k: v for k, v in request.llm.model_dump().items() if v is not None
            }
...
async def list_agent_mcps(
    ...
    try:
        mcps = await mcp_service.list_agent_mcps(
            agent_id=agent_id, include_inactive=include_inactive
        )
...
async def list_available_mcps_for_agent(
    ...
    try:
        ...
        # 获取 Agent 已启用的 MCP
        agent_mcps = await mcp_service.list_agent_mcps(agent_id=agent_id, include_inactive=True)
