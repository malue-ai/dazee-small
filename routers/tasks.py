...
from services.task_service import TaskStatus, get_task_service
...
async def list_tasks(scheduled_only: bool = Query(False, description="只返回支持定时执行的任务")):
    """
    获取已注册的后台任务列表
...
async def get_task(task_name: str):
    ...
    try:
        ...
        return APIResponse(success=True, data=task, message=f"获取任务详情: {task_name}")
...
async def run_task(task_name: str, request: RunTaskRequest):
    ...
    try:
        ...
        if result.status == TaskStatus.FAILED:
            return APIResponse(
                success=False, data=result.to_dict(), message=result.error or "任务执行失败"
            )
...
async def run_batch_task(task_name: str, request: RunBatchTaskRequest):
    ...
    try:
        ...
        if not task or not task.get("supports_scheduled"):
            raise HTTPException(status_code=400, detail=f"任务 {task_name} 不支持批量执行")

        params = request.params or {}
        params["since_hours"] = request.since_hours
        params["max_concurrent"] = request.max_concurrent

        result = await task_service.run_batch_task(task_name=task_name, params=params)

        if result.status == TaskStatus.FAILED:
            return APIResponse(
                success=False, data=result.to_dict(), message=result.error or "批量任务执行失败"
            )

        return APIResponse(
            success=True, data=result.to_dict(), message=f"批量任务 {task_name} 执行完成"
        )
