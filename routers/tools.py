...
兼容性：
- 完全兼容现有 BaseTool 接口（tools/base.py）
"""
...
@router.post(
    ...
    description="注册一个 BaseTool 子类（兼容现有工具）"
)
async def register_base_tool(
    request: BaseToolRegistrationRequest
):
    """
    注册 BaseTool 子类
    
    兼容现有的 tools/base.py 中的 BaseTool 接口
...
async def register_function_tool(
    request: FunctionToolRegistrationRequest
):
    """
    注册函数作为工具
...
@router.post(
    ...
    summary="注册 MCP 服务器",
    description="连接 MCP 服务器并自动发现注册其工具"
)
async def register_mcp_server(
    ...
    POST /api/v1/tools/mcp/register
    {
        ...
        "auth_type": "bearer",
        "auth_config": {"token_env": "MCP_OFFICE365_TOKEN"}
    }
    ```
    """
    try:
        logger.info(f"🔌 注册 MCP 服务器: {request.server_name} @ {request.server_url}")
        
        response = await tool_service.register_mcp_server(request)
...
@router.delete(
    "/mcp/{server_name}",
    summary="注销 MCP 服务器",
    description="断开 MCP 服务器连接并删除其所有工具"
)
async def unregister_mcp_server(
    ...
    try:
        ...
        success = await tool_service.unregister_mcp_server(server_name)
        
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"MCP 服务器不存在: {server_name}"
            )
        
        return JSONResponse(
            content={"success": True, "message": f"MCP 服务器 {server_name} 已注销"}
        )
    
    except Exception as e:
        logger.error(f"❌ 注销 MCP 服务器失败: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"注销失败: {str(e)}"
        )
...
@router.post(
    "/execute",
    summary="执行工具",
    description="执行指定工具，支持直接返回和流式返回"
)
...
async def unregister_tool(
    ...
    try:
        ...
        return JSONResponse(
            content={"success": True, "message": f"工具 {tool_name} 已注销"}
        )
        ...
        for tool_req in tools:
            try:
                ...
                definition = ToolDefinition(
                    ...
                    implementation={
                        "module": tool_req.module_path,
                        "class": tool_req.class_name
                    }
                )
                
                tool_service.register_tool(
                    definition=definition,
                    replace_existing=tool_req.replace_existing
                )
