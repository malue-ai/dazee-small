"""
⚠️ 已弃用：此后台任务已改为工具（Tool）方式实现

新实现位置：tools/clue_generation.py

改动原因：
- 从"每次对话完成后自动触发"改为"AI 主动判断何时调用"
- 通过工具描述告知 AI 在什么情况下使用此功能
- 提供更灵活的调用时机

如果需要使用线索生成功能，请通过工具调用 clue_generation 工具。
...
async def generate_clue_task(ctx: "TaskContext", service: "BackgroundTaskService") -> None:
    """
    线索生成任务
...
async def _generate_clues(
    ...
    try:
        ...
        assistant_preview = (
            assistant_response[:800] if len(assistant_response) > 800 else assistant_response
        )
        ...
        if session_id and event_manager:
            ...
            await event_manager.message.emit_message_delta(
                ...
                delta={"type": "clue", "content": clue_content},  # JSON 字符串格式
