...
class RagieClient:
    ...
    async def create_document_from_file(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            logger.debug(
                f"   partition={partition}, mode={mode}, file_size={len(file_content)} bytes"
            )

            async with session.post(
                url, data=data, headers={"Authorization": f"Bearer {self.api_key}"}
            ) as response:
                ...
                if response.status in [200, 201]:
                    ...
                    logger.info(
                        f"✅ Ragie 创建文档成功: id={result.get('id')}, status={result.get('status')}"
                    )
                    return result
                else:
    ...
    async def create_document_raw(
        ...
        payload = {"data": content, "name": name}  # 注意：字段名是 data 不是 content
    ...
    async def create_document_from_raw(
        ...
        return await self.create_document_raw(
            content=text, name=name, partition=partition, metadata=metadata
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=self.headers) as response:
                ...
                if response.status == 200:
                    ...
                    result = json.loads(response_text)
                    logger.info(
                        f"✅ Ragie 获取文档成功: id={result.get('id')}, status={result.get('status')}"
                    )
                    return result
                else:
                    raise Exception(f"Ragie API 错误 (HTTP {response.status}): {response_text}")
    ...
    async def get_document_chunks(
        self, document_id: str, limit: int = 100, cursor: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        获取文档的所有分块
