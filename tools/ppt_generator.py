...
class PPTGeneratorTool(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        result = {"status": "pending", "topic": topic, "phases": {}, "warnings": [], "errors": []}
        ...
        try:
            ...
            if search_queries and len(search_queries) > 0:
                ...
                search_results = await self._collect_materials(
                    queries=search_queries, time_context=time_context
                )
                ...
                result["phases"]["material_collection"]["materials"] = search_results[
                    :5
                ]  # 只存前5个
            ...
            render_result = await self._render_ppt(
                config=slidespeak_config, conversation_id=conversation_id
            )
            ...
            quality_result = self._check_quality(
                requirement=requirement, slides_config=slides_config, render_result=render_result
            )
    ...
    async def _collect_materials(
        self, queries: List[str], time_context: Optional[str]
    ) -> List[Dict[str, Any]]:
        """
        搜索并收集素材
        """
    ...
    def _plan_content(
        self, requirement: Dict[str, Any], materials: List[Dict[str, Any]], include_charts: bool
    ) -> List[Dict[str, Any]]:
        """
        规划PPT内容结构
        ...
        if custom_outline:
            ...
            for section in standard_structure:
                ...
                # 从素材中提取相关内容
                relevant_content = self._extract_relevant_content(section["title"], materials)
        ...
        if target_slides < len(base_structure):
            ...
            for i in range(extra_slides):
                base_structure.insert(
                    3 + i, {"title": f"深入分析 {i+1}", "layout": "ITEMS", "item_amount": 4}
                )
    ...
    def _create_slide_config(
        ...
        """创建单页幻灯片配置"""
        config = {"title": title, "layout": layout, "item_amount": item_amount, "content": content}
    ...
    def _generate_content_for_section(
        self, section_title: str, materials: List[Dict[str, Any]]
    ) -> str:
        """为特定章节生成内容"""
    ...
    def _extract_relevant_content(self, section_title: str, materials: List[Dict[str, Any]]) -> str:
        """从素材中提取与章节相关的内容"""
        if not materials:
            return ""
        ...
        for material in materials:
            ...
            if score > 0:
                relevant_pieces.append({"content": material.get("content", ""), "score": score})
    ...
    async def _render_ppt(
        self, config: Dict[str, Any], conversation_id: Optional[str]
    ) -> Dict[str, Any]:
    ...
    def _check_quality(
        ...
        # 3. 检查结构连贯性
        has_intro = any(
            "概述" in s.get("title", "") or "背景" in s.get("title", "") for s in slides_config
        )
        has_conclusion = any(
            "总结" in s.get("title", "") or "建议" in s.get("title", "") for s in slides_config
        )
