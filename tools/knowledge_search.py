...
class KnowledgeSearchTool(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        try:
            ...
            for doc in documents:
                logger.debug(
                    f"   📄 {doc.get('filename')} (status={doc.get('status')}, id={doc.get('document_id')})"
                )
            ...
            logger.info(
                f"🔍 Ragie 检索参数: query={query[:30]}..., top_k={top_k}, filters={combined_filters}"
            )
            ...
            filtered_chunks = [
                chunk for chunk in scored_chunks if chunk.get("score", 0) >= min_score
            ]
            ...
            return {"success": False, "error": str(e), "message": f"检索失败: {str(e)}"}
    ...
    def _build_summary(self, top_chunks: List[Dict]) -> str:
        ...
        for chunk in top_chunks:
            ...
            summary_parts.append(f"[{citation_id}] {doc_name}\n" f"{text}")
