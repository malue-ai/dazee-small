...
from services.sandbox_service import SandboxNotFoundError, SandboxServiceError, get_sandbox_service
...
class SandboxWriteFile(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "path": normalized_path, "size": result["size"]}
        except Exception as e:
...
class SandboxReadFile(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "path": normalized_path, "content": content}
...
class SandboxListFiles(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        try:
            ...
            # 转换为简单的列表格式
            files = [
                {"name": entry.name, "type": entry.type, "size": entry.size} for entry in entries
            ]

            return {"success": True, "path": normalized_path, "files": files}
        except Exception as e:
            return {"success": False, "error": str(e)}
...
class SandboxRunCommand(BaseTool):
    ...
    async def _execute_multiple(
        self, commands: list, conversation_id: str, user_id: str
    ) -> Dict[str, Any]:
        """
        批量执行多个后台命令（一键启动前后端）
        ...
        try:
            ...
            for idx, cmd_config in enumerate(commands):
                ...
                if not cmd:
                    results.append({"name": name, "success": False, "error": "命令不能为空"})
                    continue
                ...
                try:
                    await sandbox.commands.run(
                        f"{full_command} > /tmp/{name}.log 2>&1", background=True, timeout=10
                    )
                except Exception as e:
                    if "timeout" not in str(e).lower():
                        logger.warning(f"⚠️ 后台命令异常（可能正常）[{name}]: {e}")

                cmd_result = {"name": name, "success": True, "command": cmd}
    ...
    async def _execute_single(
        ...
        try:
            ...
            if background:
                ...
                try:
                    await sandbox.commands.run(
                        f"{full_command} > /tmp/app.log 2>&1", background=True, timeout=10
                    )
                except Exception as e:
                    # 后台启动可能会超时，但这是正常的
                    if "timeout" not in str(e).lower():
...
class SandboxUploadFile(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 确定 Content-Type
            content_type = self.CONTENT_TYPES.get(file_ext.lower(), "application/octet-stream")
...
class SandboxInitProject(BaseTool):
    ...
    async def execute(self, params: Dict[str, Any], context: ToolContext) -> Dict[str, Any]:
        ...
        auto_install = params.get("auto_install", True)  # 默认自动安装依赖
        user_id = params.get("user_id") or context.user_id or "default_user"
        ...
        try:
            ...
            # 并发批量写入文件
            created_files = await self._write_files_concurrent(conversation_id, files)
            ...
            # 自动安装依赖（确保模板开箱即用）
            install_result = None
            if auto_install and "package.json" in created_files:
                install_result = await self._auto_install_dependencies(conversation_id)
            ...
            result = {
                ...
                "next_step": "依赖已安装，可以直接运行 npm run dev 启动开发服务器，然后根据需求修改代码。",
            }

            # 添加依赖安装结果
            if install_result:
                result["dependencies_installed"] = install_result.get("success", False)
                if not install_result.get("success"):
                    result["install_warning"] = install_result.get(
                        "error", "依赖安装失败，请手动执行 npm install"
                    )
                    result["next_step"] = (
                        "依赖安装可能失败，请先执行 npm install，然后运行 npm run dev 启动开发服务器。"
                    )
    ...
    async def _auto_install_dependencies(self, conversation_id: str) -> Dict[str, Any]:
        """
        自动安装项目依赖

        在模板初始化后自动执行 npm install，确保项目开箱即用。
        """
        try:
            provider = get_sandbox_provider()
            sandbox = await provider._get_sandbox_obj(conversation_id)

            logger.info(f"📦 自动安装依赖: npm install")

            # 执行 npm install（设置较长的超时时间，依赖安装可能需要 1-2 分钟）
            result = await sandbox.commands.run(
                f"cd {SANDBOX_PROJECT_ROOT} && npm install", timeout=180  # 3 分钟超时
            )
            ...
            logger.warning(f"⚠️ 自动安装依赖异常: {e}")
            return {"success": False, "error": str(e)}

    async def _write_files_concurrent(
        self, conversation_id: str, files: Dict[str, str]
    ) -> list[str]:
        """
        并发批量写入文件到沙盒
        ...
        # 创建所有写入任务
        tasks = [write_single_file(path, content) for path, content in normalized_files.items()]
