...
### Step 0: 项目初始化后阅读 README.md

**重要**：当使用 `remotion` 模板初始化项目后，**必须首先阅读项目根目录的 `README.md`**，快速了解：

- 项目结构和目录约定
- 核心规则（帧驱动动画、禁止 CSS 动画）
- 可用的示例代码（`src/examples/` 目录）
- 规则文件索引（`rules/` 目录）
- 常见问题和渲染命令

```
项目初始化后：
1. 阅读 README.md → 了解项目结构和核心规则
2. 查看 src/examples/ → 参考示例代码
3. 查阅 rules/ 目录 → 获取详细规范
```
