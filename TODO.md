...
### 1. 引导页（Onboarding） （缺乏向量模型开启和配置引导、创建项目容易超时，文件夹workspace管理配置，尽量不要搞幼稚，有伤技术情怀的错误！今天完成）
...
### 2. 聊天页（Chat） （hitl、多轮对话流畅、不阻塞、workspace文件/文件夹拉拽， 集成稳定性）
...
### 3. 设置页（Settings） 只设置api key，引导页有的 配置页必须有
...
### 6. Playbook 策略学习（service端/前端交互（hitl？最好还是最成【提醒】按钮）都没做集成），记忆异步管理（基于每轮抽取基本无效，康成今天做一个方案）。  对话-emit（会话收尾，状态写入，记忆这块-》缓存（堆积记录（会话级别、 指定轮数、指定时间）））
