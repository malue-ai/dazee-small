...
class GatewayBridge:
    ...
    def _resolve_agent_id(self, msg: InboundMessage) -> Optional[str]:
        """
        ...
        if exact_key in self._binding_map:
            return self._binding_map[exact_key]

        # Try channel-level binding
        if msg.channel in self._binding_map:
            return self._binding_map[msg.channel]
    ...
    async def handle_inbound(self, msg: InboundMessage) -> None:
        ...
        try:
            ...
            conversation_id = get_cached_conversation_id(conversation_key)
            ...
            async for event in generator:
                ...
                if event_type == "content_start":
                    ...
                    # Capture the conversation_id from newly created conversations
                    conv_id = data.get("conversation_id") or data.get("conversation", {}).get("id")
                    if conv_id:
                        result_conversation_id = conv_id
                        logger.info(
                            "Conversation ID captured",
                            extra={"conversation_key": conversation_key, "conversation_id": conv_id},
                        )
            ...
            if result_conversation_id and result_conversation_id != conversation_id:
                cache_conversation_id(conversation_key, result_conversation_id)
                logger.info(
