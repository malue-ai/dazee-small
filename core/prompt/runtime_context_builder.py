...
# 检测环境并生成提示词片段
env = RuntimeContextBuilder.detect_environment()
...
class RuntimeContextBuilder:
    ...
    @classmethod
    def detect_environment(cls, detect_apps: bool = True) -> RuntimeEnvironment:
        """
        自动检测当前环境
        ...
        # 获取平台版本
        platform_version = cls._get_platform_version(system)
        ...
        installed_apps = []
        if detect_apps:
            installed_apps = cls._detect_installed_apps(system)
    ...
    @classmethod
    def _get_platform_version(cls, system: str) -> str:
        """获取平台版本"""
        try:
            if system == "darwin":
    ...
    @classmethod
    def _detect_installed_apps(cls, system: str) -> List[str]:
        """
        检测已安装的常用应用
...
# 便捷函数
def detect_and_build_environment_context(
    ...
    Returns:
        完整的环境上下文提示词
    """
    env = RuntimeContextBuilder.detect_environment(detect_apps=detect_apps)
