    ...
    def format(
        ...
        try:
            ...
            if len(result) > self.max_output_length:
                logger.warning(
                    f"⚠️ 输出超出最大长度 ({len(result)} > {self.max_output_length})，"
                    f"将被截断"
                )
...
def create_output_formatter(
    config=None,  # OutputFormatterConfig
    **kwargs
) -> OutputFormatter:
    """
    创建输出格式化器（工厂函数）
    ...
    if config:
        ...
        return OutputFormatter(
            ...
            strict_json_validation=kwargs.get("strict_json_validation", config.strict_json_validation),
