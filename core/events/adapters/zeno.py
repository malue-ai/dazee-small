...
class ZenOAdapter(EventAdapter):
    ...
    def _transform_message_start(
        self, message_id: str, conversation_id: str, timestamp: int, session_id: str = ""
    ) -> Dict[str, Any]:
        """
    ...
    def _transform_content_delta(
        self, event: Dict[str, Any], message_id: str, timestamp: int, session_id: str = ""
        ...
        if block_type == "thinking":
            ...
            logger.debug(
                f"[_transform_content_delta] 文本内容(response): len={len(text)}, accumulated_len={len(self._accumulated_content)}"
            )
            # 累积内容（用于 done 事件）
            ...
            logger.debug(
                f"[_transform_content_delta] 工具相关增量，跳过转换: block_type={block_type}"
            )
            return None
        else:
            # 未知类型，跳过
        ...
        return {
            ...
            "session_id": session_id,
            "timestamp": timestamp,
            "delta": {"type": zeno_delta_type, "content": text},
            # seq 由 EventDispatcher 统一添加
        }
    ...
    def _transform_message_delta(
        ...
        # 直接通过 message_delta 发送的类型
            ...
            "interface",
            "sandbox",
            "files",  # 系统搭建/流程图/沙箱/文件类型
            "hitl",
            "hitl_data",
            "clue",  # 人机交互类型（hitl: 表单请求, hitl_data: 用户响应, clue: 操作线索）
        ):
        ...
        return {
            ...
            "session_id": session_id,
            "timestamp": timestamp,
            "delta": {"type": zeno_delta_type, "content": zeno_content},
            # seq 由 EventDispatcher 统一添加
        }

    def _transform_message_stop(
        self, message_id: str, timestamp: int, session_id: str = ""
    ) -> Dict[str, Any]:
        """
    ...
    def _transform_session_end(
        self, event: Dict[str, Any], message_id: str, timestamp: int
    ) -> Optional[Dict[str, Any]]:
        """
        转换 session_end 事件
    ...
    def _transform_session_stopped(
        self, event: Dict[str, Any], session_id: str, timestamp: int
    ) -> Dict[str, Any]:
        """
        ...
        logger.debug(
            f"[session_stopped] 用户主动停止: session={session_id[:8] if session_id else 'N/A'}, reason={reason}"
        )
        ...
        return {
            ...
            "session_id": session_id,
            "timestamp": timestamp,
            "data": {"reason": reason, "stopped_at": stopped_at},
        }
            ...
            tasks.append(
                {
                    ...
                    "act": act,
                    "status": "pending",
                    "payload": {"request_id": request_id, "option": option},
                }
            )
    ...
    def create_intent_delta(
        self, message_id: str, intent_id: int, intent_name: str, platform: str = None
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            platform: 平台信息（可选）
        """
        intent_data = {"intent_id": intent_id, "intent_name": intent_name}
        if platform:
        ...
        return {
            ...
            "delta": {"type": "intent", "content": intent_data},
        }

    def create_preface_delta(self, message_id: str, preface_text: str) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "preface", "content": preface_text},
        }

    def create_mind_delta(
        self, message_id: str, mermaid_content: str, chart_type: str = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "mind", "content": mind_data},
        }

    def create_files_delta(self, message_id: str, files: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "files", "content": files},
        }

    def create_sql_delta(self, message_id: str, sql_query: str) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "sql", "content": sql_query},
        }

    def create_data_delta(self, message_id: str, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "data", "content": data},
        }

    def create_chart_delta(self, message_id: str, chart_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "chart", "content": chart_config},
        }
    ...
    def create_application_delta(
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "application", "content": app_data},
        }
    ...
    def create_sandbox_delta(
        ...
        return {
            ...
            "message_id": message_id,
            "timestamp": int(time.time() * 1000),
            "delta": {"type": "sandbox", "content": sandbox_data},
        }
    ...
    async def enhance_tool_result(
        ...
        logger.info(
            f"🔧 [enhance_tool_result] tool_name={tool_name}, is_error={is_error}, content_len={len(result_content) if result_content else 0}"
        )
            ...
            try:
                ...
                if result.get("url"):
                    ...
                    return self._generate_sandbox_deltas(
                        result_content, tool_input, conversation_id
                    )
    ...
    async def _generate_mcp_flowchart_deltas(self, result_content: str) -> List[Dict[str, Any]]:
        ...
        mind_data = {"flowchart_content": mermaid_content}
    ...
    def _generate_hitl_data_deltas(
        self, tool_input: Dict[str, Any], result_content: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        if result.get("timed_out"):
            ...
            logger.info(
                f"✅ 生成 hitl_data delta: 用户已响应, title={tool_input.get('title', '')[:30]}"
            )
        else:
    ...
    def _generate_analytics_deltas(self, result_content: str) -> List[Dict[str, Any]]:
        ...
        if "data" in result and isinstance(result.get("data"), dict):
            ...
            if result.get("code") not in (0, None):
                logger.warning(
                    f"⚠️ 分析类 API 返回错误码: code={result.get('code')}, msg={result.get('msg')}"
                )
                return deltas
        ...
        if dashboard_id:
            ...
            app_data = {"dashboard_id": dashboard_id, "name": report_title, "status": app_status}
    ...
    def _generate_files_deltas(self, result_content: str) -> List[Dict[str, Any]]:
        ...
        # 🔧 健壮处理：files 可能是字符串（Claude 偶发输出问题）
        if isinstance(files, str):
            logger.warning(f"⚠️ send_files files 字段是字符串，尝试解析: {files[:100]}...")
            try:
                files = json.loads(files)
            except json.JSONDecodeError:
                # 尝试修复常见格式错误：末尾多余的 }
                fixed_files = files.rstrip()
                if fixed_files.endswith("]}"):
                    fixed_files = fixed_files[:-1]  # 移除末尾多余的 }
                    try:
                        files = json.loads(fixed_files)
                        logger.info("✅ 修复 files 字符串格式成功（移除末尾多余的 }）")
                    except json.JSONDecodeError as e:
                        logger.error(f"❌ 修复后仍无法解析 files: {e}")
                        return deltas
                else:
                    logger.error(f"❌ 无法解析 files 字符串")
                    return deltas

        if not isinstance(files, list):
            logger.warning(f"⚠️ send_files files 字段不是数组: {type(files)}")
            return deltas
    ...
    def _generate_flowchart_deltas(self, result_content: str) -> List[Dict[str, Any]]:
        ...
        mind_data = {"mermaid_content": mermaid_content, "chart_type": "flowchart"}
        deltas.append(self._create_delta("mind", mind_data))
    ...
    def _create_delta(self, delta_type: str, content: Any) -> Dict[str, Any]:
        ...
        return {"type": delta_type, "content": content}
