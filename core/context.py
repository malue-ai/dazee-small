...
class Context:
    ...
    async def load_messages(self) -> List[Dict[str, Any]]:
        ...
        try:
            # 1. 加载对话元数据（获取压缩信息）
            conversation = await ConversationService.get_conversation(self.conversation_id)
            ...
            # 2. 加载消息
            db_messages = await MessageService.get_conversation_messages(
                conversation_id=self.conversation_id
            )
            
            if not db_messages:
                logger.info(f"📭 没有找到历史消息: conversation_id={self.conversation_id}")
                return []
    ...
    async def _do_compression(self, keep_recent: int):
        ...
        db_messages = await MessageService.get_conversation_messages(self.conversation_id)
        if len(db_messages) <= keep_recent:
            logger.error("数据库消息数量不足")
            return
        
        compress_from_message = db_messages[-(keep_recent + 1)]
        from_message_id = compress_from_message.id
