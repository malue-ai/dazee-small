...
class E2BMemory:
    ...
    def has_active_session(self) -> bool:
        """检查是否有活跃的临时 E2B 会话"""
        return (
            self._temporary_session is not None and 
    ...
    def get_active_sandbox(self) -> Optional[E2BSandboxSession]:
        ...
        # 查找最近使用的持久沙箱
        active_persistent = [
            s for s in self._persistent_sandboxes.values()
            if s.status == "active"
        ]
    ...
    def get_context_for_llm(self, max_history: int = 3) -> str:
        ...
        if active.installed_packages:
            context_parts.append(
                f"- 已安装包: {', '.join(active.installed_packages)}"
            )
        
        if active.files:
            context_parts.append(
                f"- 文件: {', '.join(active.files.keys())}"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "temporary_session": (
                self._temporary_session.to_dict() 
                if self._temporary_session else None
            ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "E2BMemory":
        ...
        if data.get("temporary_session"):
            memory._temporary_session = E2BSandboxSession.from_dict(
                data["temporary_session"]
            )
        
        for name, session_data in data.get("persistent_sandboxes", {}).items():
            memory._persistent_sandboxes[name] = E2BSandboxSession.from_dict(
                session_data
            )
