...
class TencentVectorDBConfig:
    ...
    # 连接配置
    url: str = field(default_factory=lambda: os.getenv("TENCENT_VDB_URL", "http://localhost:8100"))
    username: str = field(default_factory=lambda: os.getenv("TENCENT_VDB_USERNAME", "root"))
    api_key: str = field(default_factory=lambda: os.getenv("TENCENT_VDB_API_KEY", ""))

    # 数据库/集合配置
    database_name: str = field(default_factory=lambda: os.getenv("TENCENT_VDB_DATABASE", "mem0_db"))
    collection_name: str = field(
...
class Mem0Config:
    ...
    def to_mem0_config(self) -> Dict[str, Any]:
        ...
        if self.vector_store_provider == "tencent":
            ...
            vector_store_config = {"provider": "qdrant", "config": self.qdrant.to_dict()}

        return {
            "version": self.version,
            "vector_store": vector_store_config,
            "embedder": {"provider": self.embedder.provider, "config": self.embedder.to_dict()},
            "llm": {"provider": self.llm.provider, "config": self.llm.to_dict()},
        }
    ...
    def from_env(cls) -> "Mem0Config":
        ...
        Returns:
            配置好的 Mem0Config 实例
        """
        return cls(qdrant=QdrantConfig(), embedder=EmbedderConfig(), llm=LLMConfig())
