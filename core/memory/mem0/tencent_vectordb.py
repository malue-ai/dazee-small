...
class TencentVectorDB(VectorStoreBase):
    ...
    def create_col(self, name: str, vector_size: int, distance: str) -> None:
        ...
        """
        try:
            metric_map = {"COSINE": MetricType.COSINE, "L2": MetricType.L2, "IP": MetricType.IP}
            ...
            index = Index(
                ...
                FilterIndex(name="data", field_type=FieldType.String, index_type=IndexType.FILTER),
            )
    ...
    def insert(
        ...
        try:
            ...
            for vector, payload, doc_id in zip(vectors, payloads, ids):
                doc = Document(
                    id=doc_id, vector=vector, data=json.dumps(payload, ensure_ascii=False)
                )
                documents.append(doc)

            result = self.collection.upsert(documents=documents)
            affected = (
                result.get("affectedCount", 0) if isinstance(result, dict) else len(documents)
            )
    ...
    def search(
        self, query: str, vectors: List[List[float]], limit: int = 5, filters: Optional[Dict] = None
    ) -> List[OutputData]:
        """
        搜索相似向量
        ...
        try:
            ...
            search_result = self.collection.search(
                vectors=vectors, params=search_params, limit=limit, retrieve_vector=False
            )
            ...
            for inner_list in search_result:
                for doc in inner_list:
                    ...
                    results.append(OutputData(id=doc_id, score=doc_score, payload=payload))
    ...
    def update(
        ...
        try:
            if vector is not None and payload is not None:
                doc = Document(
                    id=vector_id, vector=vector, data=json.dumps(payload, ensure_ascii=False)
                )
    ...
    def get(self, vector_id: str) -> Optional[OutputData]:
        ...
        """
        try:
            result = self.collection.query(document_ids=[vector_id], retrieve_vector=False)
            ...
            if result and len(result) > 0:
                ...
                return OutputData(
                    id=doc.get("id", ""), score=doc.get("score", 1.0), payload=payload
                )
            return None
    ...
    def list(self, filters: Optional[Dict] = None, limit: Optional[int] = None) -> List[OutputData]:
        ...
        """
        try:
            result = self.collection.query(limit=limit or 100, retrieve_vector=False)
            ...
            for doc in result:
                ...
                results.append(
                    OutputData(id=doc.get("id", ""), score=doc.get("score", 1.0), payload=payload)
                )
