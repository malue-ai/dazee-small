...
FRAGMENT_EXTRACTION_PROMPT = """你是智能助理的记忆分析系统。请分析以下用户对话，提取隐性信息。
...
3. **identity_hint**：仅在用户用第一人称主动自报时提取（"我叫…""叫我…""我是XX"），不要从任务内容猜测身份
4. **区分自我标识与第三方提及**：用户提到的其他人名放入 relation_hint，不是用户自己的 identity
5. 从语气、用词、上下文推断情绪，而不是简单匹配关键词
6. **verbatim_preferences 必须原样摘录用户原话**，不要抽象化
7. 只输出 JSON，不要其他解释
...
**禁止**将对话中提到的第三方人名当作用户自己的身份。

### 关键区分：「自我标识」vs「提及他人」

identity 类只记录**正在对话的用户自己**的身份信息，判断标准是第一人称主动自报。
对话中出现的其他人名属于 relation_hint，不应存为用户的 identity。

<example>
对话: "大家都叫我老陈，帮我分析这份销售数据"
分析: 用户用第一人称自报称呼（"叫我老陈"）
long_term_memories:
  ✅ {{"content": "称呼: 老陈", "category": "identity"}}
  ❌ "正在分析销售数据" → 任务结束就没用了
</example>

<example>
对话: "帮我联系一下老陈，问问他报价多少"
分析: "老陈"是第三方被提及，不是用户自我标识
long_term_memories: [] → 老陈是第三方，不应记为用户身份
relation_hint: {{"mentioned": ["老陈"], "relationship_type": "colleague"}}
</example>

<example>
对话: "老陈说这个项目必须下周完成"
分析: "老陈"是用户转述他人的话，不是用户自我标识
long_term_memories: [] → 这里没有用户的长期身份特征
relation_hint: {{"mentioned": ["老陈"], "relationship_type": "supervisor"}}
</example>
...
对话: "我是张三，在腾讯做产品经理"
long_term_memories:
  ✅ {{"content": "姓名: 张三", "category": "identity"}}
  ✅ {{"content": "公司: 腾讯", "category": "identity"}}
  ✅ {{"content": "职业: 产品经理", "category": "identity"}}
</example>
