...
class PreferenceStability:
    ...
    evolving_preferences: Dict[str, List[str]] = field(default_factory=dict)  # 演进的偏好（随时间变化）
...
class PeriodicityAnalysis:
    """周期性分析（新增）"""
    patterns: Dict[str, Dict[str, Any]] = field(default_factory=dict)  # 周期模式 {"task_name": {"frequency": "daily", "days": [1,2,3,4,5]}}
    frequency_distribution: Dict[str, int] = field(default_factory=dict)  # 频率分布 {"daily": 3, "weekly": 1}
...
class BehaviorPattern:
    ...
    def __post_init__(self):
        ...
        if self.confidence == 0.0:
            task_confidences = [task.confidence for task in self.routine_tasks if task.confidence > 0]
            if task_confidences:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "preference_stability": {
                "stable_preferences": self.preference_stability.stable_preferences if self.preference_stability else {},
                "evolving_preferences": self.preference_stability.evolving_preferences if self.preference_stability else {},
                "preference_confidence": self.preference_stability.preference_confidence if self.preference_stability else 0.0
            } if self.preference_stability else None,
            "periodicity": {
                "patterns": self.periodicity.patterns if self.periodicity else {},
                "frequency_distribution": self.periodicity.frequency_distribution if self.periodicity else {},
                "consistency_score": self.periodicity.consistency_score if self.periodicity else 0.0
            } if self.periodicity else None,
            "conflict_detection": {
                "detected_conflicts": self.conflict_detection.detected_conflicts if self.conflict_detection else [],
                "conflict_count": self.conflict_detection.conflict_count if self.conflict_detection else 0,
                "resolved_conflicts": self.conflict_detection.resolved_conflicts if self.conflict_detection else []
            } if self.conflict_detection else None,
            # 记忆元数据
