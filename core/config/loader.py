...
class ConfigLoader:
    ...
    def _get_instance_config(self) -> Dict[str, Any]:
        ...
        if not self.instance_config_path:
            return {}
        return _load_yaml(self.instance_config_path)
    
    def _get_merged_config(self, config_name: str) -> Dict[str, Any]:
        """
        获取合并后的配置
    ...
    def get_prompt_config(self) -> Dict[str, Any]:
        """获取提示词配置"""
        return self._get_merged_config("prompt_config")
    
    def get_capabilities_config(self) -> Dict[str, Any]:
        """获取能力配置"""
        return self._get_merged_config("capabilities")
    
    def get_llm_profiles(self) -> Dict[str, Any]:
        """获取 LLM 配置"""
        return self._get_merged_config("llm_config/profiles")
    ...
    def get_module_complexity_map(self) -> Dict[str, Any]:
        """获取模块复杂度映射"""
        config = self.get_prompt_config()
        return config.get("module_complexity_map", {})
    
    def get_tool_selection_config(self) -> Dict[str, Any]:
        """获取工具选择配置"""
        config = self.get_capabilities_config()
...
def load_prompt_config(instance_name: Optional[str] = None) -> Dict[str, Any]:
    """加载提示词配置"""
    return get_config_loader(instance_name).get_prompt_config()


def load_capabilities_config(instance_name: Optional[str] = None) -> Dict[str, Any]:
    """加载能力配置"""
    return get_config_loader(instance_name).get_capabilities_config()
