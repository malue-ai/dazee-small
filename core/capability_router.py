...
class CapabilityRouter:
    ...
    def __init__(
        ...
        # 默认类型权重
        self.type_weights = {
            CapabilityType.SKILL: 10,   # Skills优先
    ...
    def _default_rules_path(self) -> str:
        """获取默认规则文件路径"""
        return str(Path(__file__).parent.parent / "config" / "routing_rules.yaml")
    ...
    def suggest_capability_type(
        ...
        基于 TOOL_CALLING_DECISION_FRAMEWORK.md 的决策树
...
def create_capability_router(
    ...
    Returns:
        配置好的CapabilityRouter实例
    """
    if registry is None:
        from .capability_registry import create_capability_registry
...
def select_tools_for_capabilities(
    ...
    """
    🆕 根据能力需求选择具体工具（Router 核心功能）
    
    这是架构的关键：将抽象能力标签映射到具体工具。
    
    Args:
        router: CapabilityRouter 实例
        required_capabilities: 能力标签列表（来自 Plan）
            例如：["web_search", "ppt_generation"]
        context: 额外上下文（可选）
    ...
    Example:
        Input: ["web_search", "ppt_generation"]
    ...
    # 2. 根据能力标签从 Registry 查找匹配的工具
    for capability_tag in required_capabilities:
        matched_tools = router.registry.find_by_capability_tag(capability_tag)
        
        if not matched_tools:
            # 没有匹配的工具，记录警告
            print(f"⚠️ No tools found for capability: {capability_tag}")
            continue
        
        # 按优先级排序（高优先级优先）
        matched_tools.sort(key=lambda c: c.priority, reverse=True)
