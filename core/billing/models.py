...
class LLMCallRecord(BaseModel):
    ...
    def total_tokens(self) -> int:
        ...
        return (
            self.input_tokens + self.output_tokens + self.thinking_tokens + self.cache_read_tokens
        )
...
class UsageResponse(BaseModel):
    ...
    prompt_tokens: int = Field(
        0, description="总输入 tokens（包含 input + cache_read + cache_write）"
    )
    ...
    llm_call_details: List[LLMCallRecord] = Field(
        default_factory=list, description="每次 LLM 调用的详细记录"
    )

    # 🆕 工具调用明细（V7.5.1 新增）
    tool_call_details: List["ToolCallRecord"] = Field(
        default_factory=list, description="工具调用详细记录（按工具聚合）"
    )
    ...
    @classmethod
    def from_tracker(cls, tracker: "EnhancedUsageTracker", latency: float = 0.0) -> "UsageResponse":
        """
        ...
        if total_prompt_tokens_for_weighting > 0:
            ...
            weighted_input_price = (
                ...
                + sum(
                    call.cache_write_tokens * call.cache_write_unit_price for call in tracker.calls
                )
        ...
        cost_saved_by_cache = sum(
            ...
            * (call.input_unit_price - call.cache_read_unit_price)
            / 1_000_000
        ...
        # 🆕 V7.5.1: 工具计费
        tool_call_details = (
            list(tracker.tool_calls.values()) if hasattr(tracker, "tool_calls") else []
        )
        ...
        llm_total = (
            total_input_price
            + total_output_price
            + total_thinking_price
            + total_cache_read_price
            + total_cache_write_price
        )
        ...
        return cls(
            ...
            + total_output
            + total_thinking,  # 包含所有输入 tokens（含缓存）
