...
from core.billing.pricing import calculate_cost, get_pricing_for_model
from core.billing.tool_pricing import calculate_tool_cost, get_tool_pricing
...
class EnhancedUsageTracker:
    ...
    def accumulate(self, llm_response, model: str = None, purpose: str = "main_response") -> None:
        ...
        # 调用新版 record_call
        self.record_call(
            llm_response=llm_response, model=model, purpose=purpose, message_id=message_id
        )
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "total_cache_creation_tokens": sum(
                c.cache_write_tokens for c in self.calls
            ),  # 注意：旧版用 creation
            "llm_calls": len(self.calls),
            # 🆕 V7.5.1: 工具调用统计
        ...
        return {
            **stats,
            ...
            "average_thinking_per_call": (
                stats["total_thinking_tokens"] / stats["llm_calls"] if stats["llm_calls"] > 0 else 0
            ),
            "cache_hit_rate": (
