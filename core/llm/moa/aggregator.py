...
"""
    ...
    async def aggregate(
        ...
        logger.info(
            f"🔄 开始聚合: strategy={strategy.value}, "
            f"source_count={len(valid_responses)}"
        )
        
        # 根据策略选择聚合方法
        if strategy == AggregationStrategy.CRITICAL_SYNTHESIS:
            return await self._critical_synthesis(
                valid_responses, original_task, task_type
            )
        elif strategy == AggregationStrategy.BEST_OF_N:
            return await self._best_of_n(
                valid_responses, original_task
            )
            ...
            return await self._weighted_merge(
