...
async def generate_failure_summary_for_multiagent(
    ...
    if orchestrator_state.status == "failed":
        has_failure = True
        stop_reason = "orchestrator_failed"
    ...
    failure_info_parts = []
    if orchestrator_state.status == "failed":
        failure_info_parts.append(f"Orchestrator 状态: {orchestrator_state.status}")
    if orchestrator_state.errors:
        for err in orchestrator_state.errors:
            failure_info_parts.append(f"错误: {err.get('error', '未知错误')}")
    if failed_agents:
        for agent in failed_agents:
            failure_info_parts.append(
                f"Agent {agent['agent_id']} 失败: {agent['error']} (轮次: {agent['turns_used']})"
            )
    if max_turns_agents:
        for agent in max_turns_agents:
            failure_info_parts.append(
