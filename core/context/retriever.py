    ...
    async def retrieve(
        ...
        for source_type, result in zip(tasks.keys(), results):
            if isinstance(result, Exception):
                logger.error(
                    f"检索失败: {source_type.value}, {str(result)}",
                    exc_info=result
                )
                ...
                logger.debug(
                    f"检索成功: {source_type.value}, "
                    f"returned {len(result)} contexts"
                )
    ...
    async def update(
        ...
        if provider:
            try:
                ...
                logger.error(
                    f"更新失败: {source_type.value}, {str(e)}",
                    exc_info=e
                )
                return False
        else:
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        for source_type, provider in self.providers.items():
            try:
                ...
                logger.error(
                    f"健康检查失败: {source_type.value}, {str(e)}",
                    exc_info=e
                )
