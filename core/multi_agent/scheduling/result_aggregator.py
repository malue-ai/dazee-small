...
class ResultAggregator:
    ...
    def __init__(self, llm_service=None, model: str = "claude-sonnet-4-5-20250929"):
        """
        初始化聚合器
    ...
    async def aggregate(self, user_query: str, worker_results: Dict[str, Any]) -> AggregationResult:
        """
        聚合 Worker 结果
    ...
    def _simple_aggregate(
        self, user_query: str, worker_results: Dict[str, Any]
    ) -> AggregationResult:
        """简单聚合（直接合并）"""
        # 按 task_id 排序
        ...
        for task_id, result in sorted_results:
            ...
            aggregated.append({"task_id": task_id, "content": content})
    ...
    async def _llm_aggregate(
        ...
        # 构建 Prompt
        prompt = AGGREGATION_PROMPT.format(user_query=user_query, worker_results=formatted_results)
...
def create_result_aggregator(llm_service=None, **kwargs) -> ResultAggregator:
    """创建聚合器"""
    return ResultAggregator(llm_service=llm_service, **kwargs)
