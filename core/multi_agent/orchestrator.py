...
class MultiAgentOrchestrator:
    ...
    async def execute(
        ...
        try:
            # ==================== Phase 1: 创建任务 ====================
            yield self._emit_event("task_created", {
                "task_id": task_id,
            ...
            if not decomposition_result.success:
                yield self._emit_event("error", {
                    "task_id": task_id,
                    "error": decomposition_result.error or "任务分解失败"
                })
                await self.fsm_engine.transition(task_id, "decompose_error", {
            ...
            yield self._emit_event("decomposition_complete", {
                "task_id": task_id,
            ...
            for sub_task in decomposition_result.sub_tasks:
                ...
                if worker_config and worker_config.system_prompt:
                    ...
                    logger.info(
                        f"⚠️ Worker '{sub_task.id}' 未找到预加载配置，使用 LLM 动态生成"
                    )
            ...
            for task_result in scheduler_result.results.values():
                yield self._emit_event("sub_task_complete", {
                    "task_id": task_id,
            ...
            if scheduler_result.completed_tasks == 0:
                await self.fsm_engine.transition(task_id, "validation_fail_final", {
                    "error": "所有子任务都失败"
                })
                yield self._emit_event("error", {
                    "task_id": task_id,
            ...
            worker_results = {
                ...
                for task_id, result in scheduler_result.results.items()
            }
            ...
            await self.fsm_engine.transition(task_id, "aggregate_complete", {
                "result": aggregation_result.final_output
            })
            ...
            yield self._emit_event("task_complete", {
                "task_id": task_id,
                "success": True,
            ...
            # 尝试转换到失败状态
            try:
                await self.fsm_engine.transition(task_id, "execution_error", {
                    "error": str(e)
                })
            except Exception:
                pass
    ...
    async def resume_task(self, task_id: str) -> AsyncGenerator[Dict, None]:
        ...
        yield self._emit_event("task_resumed", {
            "task_id": task_id,
