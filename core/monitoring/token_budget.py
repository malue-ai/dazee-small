...
class MultiAgentTokenBudget:
    ...
    def set_custom_budget(self, user_tier: str, agent_type: str, budget: int):
        """
        设置自定义预算（覆盖默认值）
    ...
    def get_budget(self, user_tier: str, agent_type: str) -> int:
        """
        获取 Token 预算
    ...
    def estimate_tokens_for_multi_agent(
        self, base_tokens: int, num_workers: int = 3, orchestration_overhead: float = 1.2
    ) -> int:
        """
        估算多智能体任务的 token 消耗
