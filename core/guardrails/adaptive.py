...
class AdaptiveGuardrails:
    ...
    def set_context(
        ...
        logger.debug(
            f"护栏上下文: complexity={self._complexity_level}, "
            f"tier={self._user_tier}"
        )
    ...
    def _get_limit(self, base_attr: str) -> float:
        ...
        return self.config.get_adjusted_limit(
            base_value,
    ...
    def suggest_adjustments(self) -> List[str]:
        ...
        if stats["tokens"]["ratio"] > 0.6:
            suggestions.append(
                f"Token 使用率 {stats['tokens']['ratio']:.0%}，"
                "建议压缩上下文"
            )
