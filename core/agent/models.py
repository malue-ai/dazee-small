...
class CriticConfig(BaseModel):
    ...
    default_action_on_timeout: CriticAction = Field(
        CriticAction.ASK_HUMAN, description="超时时的默认行动"
    )
...
async def load_multi_agent_config(
    config_path: str = "config/multi_agent_config.yaml",
) -> MultiAgentConfig:
    """
    ✅ V7.2: 异步加载多智能体配置
    ...
    if config_file.exists():
        try:
            ...
            critic_config = (
                CriticConfig(**config_data.get("critic", {})) if "critic" in config_data else None
            )
            ...
            # 获取执行配置中的超时时间（传递给每个 Agent）
            execution_config = config_data.get("execution", {})
            agent_timeout = execution_config.get("timeout_seconds", 300)
            ...
            if not agent_configs:
                agent_configs = [
                    AgentConfig(
                        ...
                        tools=["tavily_search", "exa_search", "wikipedia"],
                        timeout_seconds=agent_timeout,  # 使用配置的超时时间
                    ),
                    AgentConfig(
                ...
                # 如果配置了 agents 但没有设置 timeout_seconds，使用 execution 配置的值
                for agent in agent_configs:
                    if agent.timeout_seconds == 60:  # 只覆盖默认值
                        agent.timeout_seconds = agent_timeout
