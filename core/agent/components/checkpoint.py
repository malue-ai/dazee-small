...
class CheckpointManager:
    ...
    async def save_checkpoint(
        self, state: OrchestratorState, reason: str = "auto", last_output: Optional[str] = None
    ) -> Checkpoint:
        """
        保存检查点
    ...
    async def save_checkpoint_on_agent_completion(
        self, state: OrchestratorState, agent_id: str, result: AgentResult
    ) -> Checkpoint:
        """
        ...
        return await self.save_checkpoint(
            state=state, reason=f"agent_completed:{agent_id}", last_output=result.output
        )

    async def save_checkpoint_on_error(
        self, state: OrchestratorState, error: Exception
    ) -> Checkpoint:
        """
        在发生错误时保存检查点
        ...
        """
        return await self.save_checkpoint(state=state, reason=f"error:{type(error).__name__}")

    # ===================
    # 加载检查点
    # ===================

    async def load_latest_checkpoint(self, session_id: str) -> Optional[Checkpoint]:
        """
        加载最新的检查点
    ...
    def restore_state(self, checkpoint: Checkpoint) -> OrchestratorState:
        """
        从检查点恢复编排器状态
    ...
    def can_resume(self, checkpoint: Checkpoint) -> bool:
        """
        检查是否可以从检查点恢复
        ...
        if age_hours > 24:
            logger.warning(f"⚠️ 检查点 {checkpoint.checkpoint_id} 已过期 " f"({age_hours:.1f} 小时)")
            return False
    ...
    async def _load_session_checkpoints(self, session_id: str) -> List[Checkpoint]:
        """加载会话的所有检查点（异步）"""
        checkpoints = []
