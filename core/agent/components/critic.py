...
class CriticAgent:
    ...
    def _get_default_prompt(self) -> str:
        """获取默认提示词（从 prompts/ 目录同步加载）"""
        from prompts import load_prompt

        try:
            return load_prompt("multi_agent/critic_prompt")
        except FileNotFoundError:
            logger.warning("⚠️ Critic Prompt 文件不存在，使用最小 fallback")
            return (
                "你是评审顾问，评估执行结果并提供改进建议。"
                "输出 JSON：{observations, gaps, root_cause, suggestions, "
...
3. 建议必须具体可执行"""
        ...
        try:
            ...
            if end_pos > 0:
                ...
                try:
                    ...
                    logger.error(
                        f"❌ JSON 解析失败（截断后）: {e}, response_text={response_text[:200]}"
                    )
    ...
    def should_auto_execute(self, result: CriticResult) -> bool:
        ...
        # 低信心 + 配置要求人工介入
        if (
            result.confidence == CriticConfidence.LOW
            and self.config.require_human_on_low_confidence
        ):
            return False
