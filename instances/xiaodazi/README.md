...
2. 找到 **「云端协同」** 区块。
3. 填写：
   - **云端 URL**：默认 `https://your-cloud-agent.example.com`，也可改为自建云端地址。
...
API 文档：[Zenflux Agent API - Swagger UI](https://your-cloud-agent.example.com/docs#/chat/chat_api_v1_chat_post)。
...
2. **启动服务**（会读取 `CLOUD_URL` 并初始化云端客户端）：
   ...
   ```
   启动日志出现 `☁️ 云端客户端已配置: https://your-cloud-agent.example.com` 即表示实例化成功。

3. **API 文档**：云端 Chat 等接口见 [Zenflux Agent API - Swagger UI](https://your-cloud-agent.example.com/docs#/chat/chat_api_v1_chat_post)。
