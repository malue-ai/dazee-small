...
- **引用能力类别，不要写死 Skill 名**。用户可能安装了不同的 Skill 来提供同一种能力。正确：「配合搜索类 Skill 获取信息」「使用 PDF 读取类 Skill」。错误：「配合 ddg-search 搜索」「用 nano-pdf 读取」。如果需要举例帮助理解，用括号补充：「搜索类 Skill（如 jina-reader）」。
