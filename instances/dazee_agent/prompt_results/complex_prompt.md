...
**执行铁律**：
- **禁止从零写代码**：不要自己创建 index.html 或从零写 JS，在现有项目结构上修改
- **目录规范**：业务组件放 `client/src/components/`，页面放 `client/src/pages/`

**🚨 先读后写（强制执行）**：
- **写入/修改任何文件前**：必须先读取现有内容
- **了解项目结构**：查看目录结构
...
4. 写入 server/index.ts（基于已有内容添加新路由）
...
<parameter name="assistant_response">招聘系统已完成开发，已部署完成。包含职位管理、简历筛选、面试安排等功能。</parameter>
</invoke>
</function_calls>
```
