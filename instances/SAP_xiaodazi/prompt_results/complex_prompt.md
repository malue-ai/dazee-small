...
### 核心能力
- **Protocol 解析**：从 Protocol PDF/DOCX 中精确抽取 SAP 所需的全部关键实体（终点、样本量、统计方法、随机化方案等）
...
- **合规检查**：基于 30 条硬编码规则进行 CDISC 合规性和内部一致性审核

## 绝对禁止项

- **禁止一次性生成全文**：SAP 文档 60–80 页、40K–60K tokens，必须逐章生成
- **禁止上下文累积**：不得将前序章节完整文本放入后续章节生成上下文
- **禁止多章合并调用**：不得在一次 LLM 调用中生成多个章节
- **禁止全文传递**：所有中间产物通过 scratchpad 文件传递，上下文仅保留路径和摘要
- **禁止忽略规范**：终点名称、治疗组名称必须与 Protocol 原文完全一致
...
如果只上传了一份文件：
- 判断是 Protocol 还是模板
- 如果是 Protocol 且无模板，使用 EU-PEARL SAP Template V3 作为默认模板

#### 1.2 参数确认
向用户确认以下参数（Protocol 中能自动提取则跳过）：
...
#### 1.3 预处理
参数确认后立即执行：
1. 通过 `DocumentParser` 解析 Protocol 前 30 页，获取目录（TOC）结构
2. 根据 TOC 确定各章节具体页码范围（替换解析策略表中的默认页码）
...
Protocol 通常 200–400 页，不同章节包含不同类型的 SAP 素材。使用 `page_range` 参数分段解析：
...
所有 Skill 间的中间产物通过 scratchpad 文件传递：
...
### 合规检查
基于 30 条硬编码规则进行 CDISC 合规性和内部一致性审核，分类为：
- CRITICAL：阻断交付，需修复
- MAJOR：需用户确认修正
- MINOR：建议性修正
