...
class SandboxServicer(pb2_grpc.SandboxServiceServicer):
    ...
    async def RunProject(
        self,
        request: pb2.SandboxRunProjectRequest,
        context: grpc.aio.ServicerContext
    ) -> pb2.SandboxRunProjectResponse:
        """运行项目"""
        try:
            service = get_sandbox_service()
            ...
            return pb2.SandboxRunProjectResponse(
            ...
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(str(e))
            return pb2.SandboxRunProjectResponse(success=False, error=str(e))
        except SandboxServiceError as e:
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(str(e))
            return pb2.SandboxRunProjectResponse(success=False, error=str(e))
            ...
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details("运行项目失败")
            return pb2.SandboxRunProjectResponse(success=False, error="运行项目失败")
    
    async def StopProject(
        self,
        request: pb2.SandboxStopProjectRequest,
        context: grpc.aio.ServicerContext
    ) -> pb2.SandboxOperationResponse:
        """停止项目"""
        try:
            service = get_sandbox_service()
            success = await service.stop_project(request.conversation_id)
            
            return pb2.SandboxOperationResponse(
                success=success,
                message="项目已停止" if success else "停止项目失败"
            )
            ...
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details("停止项目失败")
            return pb2.SandboxOperationResponse(success=False, message="停止项目失败")
    
    async def GetLogs(
        self,
        request: pb2.SandboxLogsRequest,
        context: grpc.aio.ServicerContext
    ) -> pb2.SandboxLogsResponse:
        """获取项目日志"""
        try:
            service = get_sandbox_service()
            lines = request.lines if request.lines > 0 else 100
            logs = await service.get_logs(request.conversation_id, lines)
            
            return pb2.SandboxLogsResponse(logs=logs)
        
        except SandboxNotFoundError as e:
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(str(e))
            return pb2.SandboxLogsResponse()
        except SandboxServiceError as e:
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(str(e))
            return pb2.SandboxLogsResponse()
